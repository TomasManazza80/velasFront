...
const ConfiguracionMayorista = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const cartTotalMinConf = globalConfigs.find(
        (c) => c.key === "wholesale_cart_total_min",
      );
      const productQtyMinConf = globalConfigs.find(
        (c) => c.key === "wholesale_product_qty_min",
      );

      setConfig({
        cartTotalMin: cartTotalMinConf ? parseFloat(cartTotalMinConf.value) : 0,
        productQtyMin: productQtyMinConf
          ? parseFloat(productQtyMinConf.value)
          : 0,
      });
  ...
  const saveConfig = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/gastos/global-configs`, {
        configs: [
          {
            key: "wholesale_cart_total_min",
            value: config.cartTotalMin,
            description: "Monto mínimo de carrito para precio mayorista",
          },
          {
            key: "wholesale_product_qty_min",
            value: config.productQtyMin,
            description: "Cantidad mínima de producto para precio mayorista",
          },
        ],
      });
  ...
  const toggleProductWholesale = async (product) => {
    try {
      ...
      const updatedProducts = products.map((p) =>
        p.id === product.id
          ? { ...p, aplicarMayoristaPorCantidad: updatedValue }
          : p,
      );
  ...
  return (
    <div
      className="text-black bg-white min-h-screen p-6 max-w-6xl mx-auto pb-32"
      style={{ fontFamily: '"Inter", sans-serif' }}
    >
      <div className="mb-8">
        <h2 className="text-3xl text-black mb-2 font-bold tracking-tight uppercase">
          CONFIGURACIÓN MAYORISTA
        </h2>
        <p className="text-gray-500 text-xs tracking-widest uppercase font-medium">
          DEFINIR REGLAS DE NEGOCIO PARA COMPRAS AL POR MAYOR
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="p-6 rounded-2xl border border-gray-200 bg-white shadow-sm">
            <h3 className="text-lg mb-6 flex items-center gap-2 font-bold tracking-tight uppercase text-black">
              <FiDollarSign className="text-black" />
              LÍMITES GLOBALES
            </h3>
            <form onSubmit={saveConfig} className="space-y-6">
              <div>
                <label className="text-[10px] uppercase text-gray-500 block mb-2 font-bold">
                ...
                <p className="text-[10px] text-gray-500 mt-2 leading-relaxed">
                  Si el total del carrito supera este monto, se aplicará precio
                  mayorista a{" "}
                  <strong className="text-black uppercase">todos</strong> los
                  productos.
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-gray-500 mt-2 leading-relaxed">
                  Cantidad mínima para activar precio mayorista en productos
              ...
              <button
                type="submit"
                disabled={saving}
                className="w-full py-3 bg-[#F4F7FE] text-gray-900 font-bold uppercase text-xs rounded-xl hover:bg-gray-800 transition-all flex items-center justify-center gap-2"
              >
                {saving ? (
                  "GUARDANDO..."
                ) : (
                  <>
                    <FiSave /> GUARDAR REGLAS
                  </>
                )}
              </button>
            </form>
        ...
        <div className="md:col-span-2">
          <div className="p-6 rounded-2xl border border-gray-200 bg-white shadow-sm h-full flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg flex items-center gap-2 font-bold tracking-tight uppercase text-black">
            ...
            <p className="text-[10px] text-gray-600 mb-4 bg-gray-100 p-3 rounded-lg border border-gray-200 flex items-center">
              ...
              <span>
                Selecciona los productos que aplicarán precio mayorista cuando
                superen las{" "}
                <strong className="text-black">
                  {config.productQtyMin} unidades
                </strong>
                .
              </span>
            </p>
            ...
            <div className="overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
              {loading ? (
                <p className="text-center text-gray-400 py-10 text-xs animate-pulse font-bold tracking-widest uppercase">
                  CARGANDO INVENTARIO...
                </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase transition-all ${product.aplicarMayoristaPorCantidad ? "bg-[#F4F7FE] text-gray-900" : "bg-white border border-gray-300 text-gray-500 hover:text-black hover:border-black"}`}
                        >
                ...
                <p className="text-center text-gray-400 py-10 text-[10px] font-bold tracking-widest uppercase">
