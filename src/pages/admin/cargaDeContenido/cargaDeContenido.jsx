...
const STYLES = {
  ...
  label:
    "font-['Inter'] text-[10px] md:text-[11px] font-bold text-white uppercase tracking-[0.2em] mb-2 block",
  tech: "font-['Inter'] tracking-widest uppercase",
  glass: "bg-white/[0.02] backdrop-blur-xl border border-white/10",
  input:
    "bg-black/20 border border-white/10 p-3 w-full text-sm font-['Inter'] placeholder:text-zinc-600 placeholder:uppercase focus:border-white focus:outline-none transition-colors",
  buttonAction:
    "bg-white text-black font-['Inter'] font-[900] uppercase tracking-widest transition-all hover:bg-zinc-200 shadow-[0_0_20px_rgba(255,255,255,0.1)]",
...
const SECCIONES = {
  PROMOS: {
    ...
    aspect: "16:9",
    icon: FiActivity,
    putUrl: `${API_URL}/contenido/actualizarContenidoVisual/2`,
  },
  REPARACIONES: {
    id: 3,
    label: "MOD_REPARACIONES",
    aspect: "4:3",
    icon: FiLayers,
    putUrl: `${API_URL}/contenido/actualizarContenidoVisual/3`,
  },
};
...
const FOOTER_CONFIG = {
  ...
  icon: FiMonitor,
  endpoints: {
    get: `${API_URL}/api/footer/list`,
    put: `${API_URL}/api/footer`,
  },
};
...
const CargaContenidoWeb = () => {
  ...
  const [selectedFooterKey, setSelectedFooterKey] = useState("");
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const response = await fetch(
          `${API_URL}/contenido/obtenerContenidoVisual`,
        );
        const data = await response.json();
        const map = {};
        const posMap = {};
    ...
    const fetchHeroSlides = async () => {
      try {
        ...
        console.error("HERO_SLIDE_FETCH_ERROR", error);
      }
    };

    const fetchFooter = async () => {
      try {
        const response = await fetch(FOOTER_CONFIG.endpoints.get);
        if (response.ok) {
          const data = await response.json();
          setFooterItems(data);
        }
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      if (!file.type.startsWith("image/")) {
        setFileError(
          "FORMATO NO VÁLIDO: POR FAVOR SUBIR UNA IMAGEN (JPG, PNG, WEBP).",
        );
        return;
      }
      setPreviewFile(file);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!ikRes.ok) {
        ...
        throw new Error(
          `IMAGEKIT_UPLOAD_FAILED: ${errorData.message || ikRes.statusText}`,
        );
      }
      ...
      if (response.ok) {
        ...
        throw new Error(
          `BACKEND_SYNC_FAILED: ${backError.message} | ${backError.detail || ""}`,
        );
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        `¿ESTÁS SEGURO DE ELIMINAR EL ASSET EN ${section.label}? ESTA ACCIÓN NO SE PUEDE DESHACER.`,
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `${API_URL}/contenido/eliminarContenidoVisual/${section.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleSuccessCaseUpload = async () => {
    if (
      ...
      !newCaseData.equipo ||
      !newCaseData.falla ||
      !newCaseData.resultado
    ) {
      alert(
        "SISTEMA: Por favor, complete todos los campos y seleccione una imagen.",
      );
      return;
    }
  ...
  const handleSuccessCaseDelete = async (caseId) => {
    if (
      !confirm(
        "¿ESTÁS SEGURO DE ELIMINAR ESTE CASO DE ÉXITO? LA ACCIÓN ES IRREVERSIBLE.",
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `${CASOS_EXITO_CONFIG.endpoints.delete}/${caseId}`,
        { method: "DELETE" },
      );
      if (response.ok) {
  ...
  const handleHeroSlideUpload = async () => {
    if (
      !previewFile ||
      !newHeroSlideData.title ||
      !newHeroSlideData.subtitle ||
      !newHeroSlideData.label
    ) {
      alert(
        "SISTEMA: Por favor, complete todos los campos y seleccione una imagen.",
      );
    ...
    try {
      ...
      const slidePayload = {
        ...newHeroSlideData,
        image: secureUrl,
        position: `center ${verticalOffset}%`,
      };
  ...
  const handleHeroSlideDelete = async (slideId) => {
    ...
    try {
      const response = await fetch(
        `${HERO_SLIDER_CONFIG.endpoints.delete}/${slideId}`,
        { method: "DELETE" },
      );
      if (response.ok) {
  ...
  const handleFooterSubmit = async () => {
    if (!selectedFooterKey || !footerData.title || !footerData.content) {
      alert("SISTEMA: Seleccione una sección y complete el título y contenido.");
      return;
    }
    setIsSubmittingFooter(true);
    try {
      const response = await fetch(FOOTER_CONFIG.endpoints.put, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          key: selectedFooterKey,
      ...
      if (response.ok) {
        ...
        setFooterItems((prev) => {
          const exists = prev.find((item) => item.key === selectedFooterKey);
          if (exists) {
            return prev.map((item) => (item.key === selectedFooterKey ? updatedItem : item));
          }
          return [...prev, updatedItem];
        });
        alert("SISTEMA: SECCIÓN_DE_FOOTER_ACTUALIZADA");
      } else {
        throw new Error("BACKEND_SYNC_FAILED");
      }
    } catch (error) {
  ...
  if (loading)
    return (
      <div className="h-screen bg-black flex items-center justify-center text-white font-['Inter'] tracking-[0.2em] animate-pulse">
        SINCRONIZANDO_CORE...
      </div>
    );
  ...
  return (
    <div className="p-4 md:p-10 bg-black min-h-screen text-white font-['Inter'] space-y-6 md:space-y-12">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 md:gap-8 border-b border-white/5 pb-6 md:pb-10">
        <div>
          <h2 className={STYLES.title + " text-2xl md:text-4xl"}>
            <FiGlobe className="inline text-white mr-4" />
            CMS_<span className="text-white">VISUAL_CORE</span>
          </h2>
          <p
            className={
              STYLES.tech +
              " text-[8px] md:text-[10px] text-zinc-500 mt-2 md:mt-4 tracking-[0.3em] md:tracking-[0.5em]"
            }
          >
            IMAGEKIT_GATEWAY_ACTIVE
          </p>
        </div>
        ...
        <div
          className={
            STYLES.glass +
            " p-1 flex overflow-x-auto max-w-full no-scrollbar gap-1"
          }
        >
          ...
          <button
            key={FOOTER_CONFIG.id}
            onClick={() => {
              setSeccionDestino(FOOTER_CONFIG.id);
              setPreviewUrl(null);
              setPreviewFile(null);
            }}
            className={`whitespace-nowrap px-4 md:px-6 py-2 md:py-3 text-[9px] md:text-[10px] font-black tracking-widest uppercase transition-all ${seccionDestino === FOOTER_CONFIG.id ? STYLES.tabActive : STYLES.tabInactive}`}
          >
            {FOOTER_CONFIG.label}
          </button>
        </div>
        ...
        <div className="grid grid-cols-12 gap-6 md:gap-12">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6 md:space-y-8">
            <section className="space-y-4">
              ...
              <div
                className={`relative h-60 md:h-80 border border-dashed transition-all flex flex-col items-center justify-center ${previewUrl ? "border-white" : "border-zinc-800 bg-white/[0.01]"}`}
              >
                {!previewUrl ? (
                  <>
                    ...
                    <label
                      className={
                        STYLES.buttonAction +
                        " px-8 py-4 text-[10px] cursor-pointer"
                      }
                    >
                      SELECCIONAR_ARCHIVO
                      <input
                  ...
                  <div className="relative w-full h-full p-4 animate-in fade-in">
                    ...
                    <button
                      ...
                      className="absolute top-4 md:top-8 right-4 md:right-8 p-3 bg-black/80 text-white border border-zinc-700 hover:bg-white hover:text-black transition-colors"
                    >
                      <FiTrash2 />
                    </button>
                  </div>
            ...
            <section className={STYLES.glass + " p-4 md:p-8 space-y-6"}>
              <div className="space-y-4">
                ...
                <div className="text-center font-['Inter'] font-bold text-[9px] text-white">
                  {verticalOffset}%
                </div>
              </div>
              <div className="flex justify-between items-center bg-black p-4 border border-white/5">
                ...
                <span
                  className={STYLES.tech + " text-[9px] text-white font-bold"}
                >
                  ID_{SECCIONES[seccionDestino].id}
                </span>
              </div>
              <div className="flex flex-col gap-4">
                <button
                  ...
                  className={`w-full py-4 md:py-5 text-sm ${STYLES.buttonAction} flex items-center justify-center gap-4 ${(!previewUrl || subiendo) && "opacity-20 grayscale"}`}
                >
                  {subiendo ? (
                ...
                <button
                  ...
                  disabled={
                    !dbContent[SECCIONES[seccionDestino].id] || subiendo
                  }
                  className={`w-full py-3 text-[10px] font-black tracking-widest uppercase border border-zinc-600 text-zinc-400 hover:bg-white hover:text-black transition-all ${(!dbContent[SECCIONES[seccionDestino].id] || subiendo) && "opacity-10 grayscale"}`}
                >
          ...
          <div className="col-span-12 lg:col-span-7 space-y-4 md:space-y-6">
            <h3
              className={
                STYLES.tech +
                " text-[10px] md:text-[11px] text-zinc-500 flex items-center gap-3"
              }
            >
              <FiMaximize2 /> MONITOR_REAL_TIME
            </h3>
            <div className="relative bg-zinc-950 border border-zinc-900 shadow-2xl h-[400px] md:h-[550px] overflow-hidden">
              <div className="bg-zinc-900 p-3 md:p-4 border-b border-white/5 flex justify-between items-center">
                <div
                  className={
                    STYLES.tech +
                    " text-[8px] md:text-[9px] text-zinc-400 truncate max-w-[200px] md:max-w-md"
                  }
                >
                  {previewUrl
                    ? "VISTA_PREVIA_LOCAL"
                    : `PROD_URL: ${dbContent[SECCIONES[seccionDestino].id] || "SIN_ASIGNAR"}`}
                </div>
              </div>
                ...
                {previewUrl || dbContent[SECCIONES[seccionDestino].id] ? (
                  <div className="w-full relative group">
                    <img
                      ...
                      className={`w-full object-cover transition-all duration-700 grayscale ${previewUrl ? "border-2 border-white scale-[1.01]" : "border border-white/10 opacity-80"}`}
                      style={{
                        aspectRatio: SECCIONES[seccionDestino].aspect.replace(
                          ":",
                          "/",
                        ),
                        objectPosition: previewUrl
                          ? `center ${verticalOffset}%`
                          : dbPositions[SECCIONES[seccionDestino].id] ||
                            "center center",
                      }}
                      alt="Output"
                    />
                    {previewUrl && (
                      <div className="absolute top-2 md:top-4 right-2 md:right-4 bg-white text-black text-[8px] md:text-[9px] font-black px-2 md:px-4 py-1 animate-pulse uppercase">
                        PENDIENTE
                      </div>
                    )}
                  </div>
        ...
        <div className="grid grid-cols-12 gap-6 md:gap-12">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6 md:space-y-8">
            <section className="space-y-4">
              ...
              <div
                className={`relative h-48 md:h-60 border border-dashed transition-all flex flex-col items-center justify-center ${previewUrl ? "border-white" : "border-zinc-800 bg-white/[0.01]"}`}
              >
                {!previewUrl ? (
                  <>
                    <FiUploadCloud
                      size={30}
                      className="text-zinc-600 mb-4 md:mb-6"
                    />
                    <label
                      className={
                        STYLES.buttonAction +
                        " px-6 py-3 text-[9px] cursor-pointer"
                      }
                    >
                      SELECCIONAR_IMAGEN
                      <input
                  ...
                  <div className="relative w-full h-full p-3 animate-in fade-in">
                    <img
                      src={previewUrl}
                      className="w-full h-full object-cover border border-white"
                      alt="Preview"
                    ...
                    <button
                      ...
                      className="absolute top-4 right-4 p-2 bg-black/80 border border-zinc-700 text-white hover:bg-white hover:text-black transition-colors"
                    >
                      <FiTrash2 size={16} />
                    </button>
                  </div>
            ...
            <section className={STYLES.glass + " p-6 md:p-8 space-y-6"}>
              ...
              <div className="space-y-4">
                ...
                <textarea
                  name="resultado"
                  value={newCaseData.resultado}
                  onChange={handleNewCaseChange}
                  placeholder="EJ: SE REEMPLAZÓ EL IC DE CARGA..."
                  className={STYLES.input + " h-20 md:h-24"}
                  rows="3"
                ></textarea>
              </div>
              <button
                onClick={handleSuccessCaseUpload}
                disabled={
                  isSubmittingCase || !previewFile || !newCaseData.equipo
                }
                className={`w-full py-4 md:py-5 text-sm ${STYLES.buttonAction} flex items-center justify-center gap-4 ${(isSubmittingCase || !previewFile || !newCaseData.equipo) && "opacity-20 grayscale"}`}
              >
                {isSubmittingCase ? (
                  <FiRefreshCw className="animate-spin" />
                ) : (
          ...
          <div className="col-span-12 lg:col-span-7 space-y-6">
            ...
            >
              <FiAward /> HISTORIAL ({casosExito.length})
            </h3>
            <div className="relative bg-zinc-950 border border-zinc-900 shadow-2xl h-[500px] md:h-[700px] overflow-y-auto p-4 space-y-4">
              {casosExito.length === 0 ? (
                ...
                casosExito.map((caso) => (
                  <div
                    key={caso.id}
                    className="flex flex-col md:flex-row items-start gap-4 p-4 bg-black border border-white/5 rounded-lg animate-in fade-in"
                  >
                    <img
                      src={caso.imagen}
                      className="w-full md:w-24 h-48 md:h-24 object-cover grayscale rounded-md border border-white/10"
                      alt={caso.equipo}
                    />
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h4 className="font-black text-base text-white uppercase tracking-tighter">
                          {caso.equipo}
                        </h4>
                        <button
                          onClick={() => handleSuccessCaseDelete(caso.id)}
                          className="md:hidden p-2 text-zinc-600 hover:bg-white hover:text-black rounded-lg transition-all"
                        >
                          <FiTrash2 size={16} />
                        </button>
                      </div>
                      <p className="text-[9px] md:text-[10px] font-bold tracking-[0.2em] text-white uppercase mt-1">
                        FALLA: {caso.falla}
                      </p>
                      <p className="text-xs text-zinc-400 mt-2 md:mt-3 italic">
                        "{caso.resultado}"
                      </p>
                    </div>
                    <button
                      onClick={() => handleSuccessCaseDelete(caso.id)}
                      className="hidden md:block p-3 text-zinc-600 hover:bg-white hover:text-black rounded-lg transition-all"
                    >
                      <FiTrash2 size={16} />
                    </button>
                  </div>
      ...
      {seccionDestino === HERO_SLIDER_CONFIG.id && (
        <div className="grid grid-cols-12 gap-6 md:gap-12">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6 md:space-y-8">
                ...
                {!previewUrl ? (
                  ...
                  <div className="relative w-full h-full p-3 animate-in fade-in">
                    <img
                      src={previewUrl}
                      className="w-full h-full object-cover border border-white"
                      style={{ objectPosition: `center ${verticalOffset}%` }}
                      alt="Preview"
                    />
                    <button
                      onClick={() => {
            ...
            <section className={STYLES.glass + " p-6 md:p-8 space-y-6"}>
              ...
              <div className="space-y-4">
                <input
                  name="label"
                  value={newHeroSlideData.label}
                  onChange={handleNewHeroSlideChange}
                  placeholder="ETIQUETA (Ej: LU PETRUCCELLI)"
                  className={STYLES.input}
                />
                <input
                  name="title"
                  value={newHeroSlideData.title}
                  onChange={handleNewHeroSlideChange}
                  placeholder="TÍTULO (Ej: ELEGANCE & STYLE)"
                  className={STYLES.input}
                />
                <input
                  name="subtitle"
                  value={newHeroSlideData.subtitle}
                  onChange={handleNewHeroSlideChange}
                  placeholder="SUBTÍTULO (Ej: Handmade collection)"
                  className={STYLES.input}
                />
              </div>
              ...
              <button
                ...
                className={`w-full py-4 md:py-5 text-sm ${STYLES.buttonAction} flex items-center justify-center gap-4 ${(isSubmittingSlide || !previewFile || !newHeroSlideData.title || !newHeroSlideData.subtitle || !newHeroSlideData.label) && "opacity-20 grayscale"}`}
              >
                {isSubmittingSlide ? (
                  <FiRefreshCw className="animate-spin" />
                ) : (
          ...
          <div className="col-span-12 lg:col-span-7 space-y-6">
            ...
            >
              <FiMonitor /> SLIDES CONFIGURADOS ({heroSlides.length})
            </h3>
            <div className="relative bg-zinc-950 border border-zinc-900 shadow-2xl h-[500px] md:h-[700px] overflow-y-auto p-4 space-y-4">
              {heroSlides.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center opacity-10 text-center">
                ...
                heroSlides.map((slide) => (
                  ...
                  >
                    <img
                      src={slide.image}
                      className="w-full md:w-48 h-48 md:h-32 object-cover rounded-md border border-white/10"
                      style={{ objectPosition: slide.position || "center" }}
                      alt={slide.title}
                    />
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <h4 className="font-black text-xl text-white uppercase tracking-tighter">
                          {slide.title}
                        </h4>
                        <button
      ...
      {seccionDestino === FOOTER_CONFIG.id && (
        <div className="grid grid-cols-12 gap-6 md:gap-12">
          {/* PANEL DE CONTROL FOOTER */}
          <div className="col-span-12 lg:col-span-5 space-y-6 md:space-y-8">
            <section className="space-y-4">
              <label className={STYLES.label}>SECCIÓN A EDITAR</label>
              <select
                className={STYLES.input}
                value={selectedFooterKey}
                onChange={(e) => {
                  const key = e.target.value;
                  setSelectedFooterKey(key);
                  const existing = footerItems.find(item => item.key === key);
                  if (existing) {
                    setFooterData({ title: existing.title, content: existing.content });
                  } else {
                    setFooterData({ title: "", content: "" });
                  }
                }}
              >
                ...
                <option value="tiendas">Nuestras Tiendas Exclusivas</option>
                <option value="trabajo">Trabaja con Nosotros</option>
                <option value="cuidado">Cuidado del Producto</option>
                <option value="mayoristas">Mayoristas</option>
                <option value="compra">Información para tu Compra</option>
                <option value="terminos">Términos y Condiciones</option>
                <option value="arrepentimiento">Botón de Arrepentimiento</option>
                <option value="aromatizadores">Aromatizadores Ultrasónicos</option>
                <option value="difusores">Difusores de ambiente</option>
                <option value="velas">Velas aromáticas cera de soja</option>
                <option value="productos">Todos los productos</option>
              </select>

              {selectedFooterKey && (
                <>
                  <label className={STYLES.label}>TÍTULO DEL MODAL</label>
                  <input
                    type="text"
                  ...
                  <label className={STYLES.label}>CONTENIDO TEXTUAL</label>
                  <textarea
                    value={footerData.content}
                    onChange={(e) => setFooterData(prev => ({ ...prev, content: e.target.value }))}
                    className={STYLES.input + " h-48 resize-none"}
                    placeholder="Escriba el contenido detallado..."
                  />

                  <button
                    onClick={handleFooterSubmit}
                    disabled={isSubmittingFooter || !footerData.title || !footerData.content}
                    className={`w-full py-4 md:py-5 text-sm ${STYLES.buttonAction} flex items-center justify-center gap-4 ${(isSubmittingFooter || !footerData.title || !footerData.content) && "opacity-20 grayscale"}`}
                  >
                    {isSubmittingFooter ? <FiRefreshCw className="animate-spin" /> : <FiPlusCircle />}
                    {isSubmittingFooter ? "GUARDANDO..." : "GUARDAR CAMBIOS"}
                  </button>
                </>
              )}
            </section>
          ...
          {/* LISTADO DE SECCIONES CONFIGURADAS */}
          <div className="col-span-12 lg:col-span-7 space-y-6">
            <h3 className={STYLES.tech + " text-[10px] md:text-[11px] text-zinc-500 flex items-center gap-3"}>
              <FiMonitor /> SECCIONES CONFIGURADAS ({footerItems.length})
            </h3>
            <div className="relative bg-zinc-950 border border-zinc-900 shadow-2xl h-[500px] md:h-[700px] overflow-y-auto p-4 space-y-4">
              {footerItems.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center opacity-10 text-center">
                  <FiMonitor size={60} />
                  <p className={STYLES.tech + " mt-4"}>SIN SECCIONES PERSONALIZADAS</p>
                </div>
              ) : (
                footerItems.map((item) => (
                  <div key={item.id} className="flex flex-col md:flex-row items-start gap-4 p-4 bg-black border border-white/5 rounded-lg">
                    <div className="flex-1">
                      <h4 className="font-black text-xl text-white uppercase tracking-tighter">{item.title}</h4>
                      <p className="text-[9px] md:text-[10px] font-bold tracking-[0.2em] text-[#cba394] uppercase mt-1">KEY: {item.key}</p>
                      <p className="text-xs text-zinc-400 mt-2 line-clamp-3">{item.content}</p>
                    </div>
                  </div>
                ))
              )}
