...
const styles = {
  overlay:
    "fixed inset-0 z-[120] flex items-center justify-center bg-black/90 backdrop-blur-md p-4",
  modal:
    "bg-[#0A0A0A] border border-white/10 w-full max-w-5xl h-[90vh] overflow-hidden flex flex-col shadow-[0_0_50px_rgba(255,140,0,0.15)]",
  header:
    "p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]",
  title:
    "font-['Montserrat'] font-black text-xl uppercase tracking-tighter text-white",
  body: "flex-1 overflow-y-auto p-8 custom-scrollbar",
  section: "mb-10",
  sectionTitle:
    "text-[10px] font-black text-orange-500 uppercase tracking-[0.4em] mb-6 flex items-center gap-3 border-l-2 border-orange-500 pl-4",
  ...
  variantCard:
    "border border-white/5 bg-white/[0.02] p-4 rounded-xl hover:border-orange-500/30 transition-all",
  ...
  const fetchByTitle = async (title) => {
    ...
    try {
      ...
      const found = data.find(
        (p) => p.nombre.toLowerCase() === cleanTitle.toLowerCase(),
      );
      if (found) {
        setProduct(found);
      } else {
              ...
              <div>
                <h2 className={styles.title}>
                  {loading
                    ...
                    : product?.nombre || "DETALLES DEL ARTÍCULO"}
                </h2>
                <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">
                  {product?.id ? `ID: ${product.id}` : "INFO TERMINAL"}
                </span>
              </div>
            </div>
            <button
          ...
          <div className={styles.body}>
            {loading ? (
              <div className="h-full flex flex-col items-center justify-center gap-4 py-20">
                ...
                <p className="font-['JetBrains_Mono'] text-orange-500 text-xs animate-pulse">
              ...
              <div className="p-10 text-center border border-red-500/20 bg-red-500/5 rounded-xl">
                ...
                <p className="text-red-500 fedecell-tech text-sm uppercase font-black">
                  {error}
                </p>
                <p className="text-zinc-500 text-xs mt-2">
                  DATO RECIBIDO: {productData}
                </p>
              </div>
            ) : (
              product && (
                <div className="space-y-12">
                            ...
                            {product.imagenes.length > 1 && (
                              <div className="absolute inset-x-2 top-1/2 -translate-y-1/2 flex justify-between z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  ...
                                  className="p-2 bg-black/50 rounded-full hover:bg-orange-500 transition-all"
                                >
                                  <FiChevronLeft />
                                </button>
                                <button
                      ...
                      <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                        {product.imagenes?.map((img, i) => (
                          <button
                            ...
                            className={`w-12 h-12 flex-shrink-0 border-2 rounded-lg p-1 bg-black ${currentImg === i ? "border-orange-500" : "border-white/5 opacity-50"}`}
                          >
                            <img
                  ...
                  {product.variantes?.length > 0 && (
                    <div>
                      <h3 className={styles.sectionTitle}>
                        03. Configuración de Lotes y Precios
                      </h3>
                        ...
                        {product.variantes.map((v, i) => (
                          <div key={i} className={styles.variantCard}>
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div
                                className={`px-2 py-1 rounded text-[10px] font-black ${v.stock > product.alerta ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
                              >
                                STOCK: {v.stock}
                              </div>
                            </div>
