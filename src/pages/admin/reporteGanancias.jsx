...
const ReporteGanancias = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const fixedRes = await axios.get(
        `${API_URL}/gastosMensuales/obtenerGastosMensuales`,
      );
  ...
  const totalFixedExpenses = expensesData.fixed.reduce(
    (acc, item) => acc + parseFloat(item.monto),
    0,
  );
  const totalVariableExpenses = expensesData.variable.reduce(
    (acc, item) => acc + parseFloat(item.monto),
    0,
  );
  ...
  const generatePDF = () => {
    ...
    addRow(
      ...
      -reportData.resumen.totalCostos,
      false,
      [100, 100, 100],
    );
    y += 2;
    ...
    y += 5;
    addRow(
      "Gastos Fijos (Alquileres, Salarios)",
      -totalFixedExpenses,
      false,
      [100, 100, 100],
    );
    ...
    if (expensesData.fixed.length > 0 || expensesData.variable.length > 0) {
      ...
      const addExpenseTable = (title, items, nameKey, dateKey) => {
        ...
        items.forEach((item) => {
          ...
          doc.text(
            new Date(item[dateKey] || item.createdAt).toLocaleDateString(),
            15,
            y,
          );
      ...
      addExpenseTable(
        "EGRESOS VARIABLES (Caja/Operativos)",
        expensesData.variable,
        "detalle",
        "createdAt",
      );
    }
    ...
    if (reportData.productos.length > 0) {
      ...
      reportData.productos.forEach((prod) => {
        ...
        doc.text(formatCurrency(prod.precioVentaTotal), 160, y, {
          align: "right",
        });
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto p-4 font-['Inter']">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-10 border-b border-white/10 pb-6 gap-6">
        <div>
          <h2 className="font-['Inter'] font-[900] uppercase tracking-tighter text-white text-3xl md:text-4xl flex items-center gap-4">
            <FiTrendingUp className="text-white" /> REPORTE DE{" "}
            <span className="text-white">GANANCIAS</span>
          </h2>
      ...
      <div className="bg-zinc-900/50 border border-white/5 p-6 mb-10 flex flex-col md:flex-row gap-6 items-end">
        <div className="w-full md:w-auto flex-1">
          <label className="font-['Inter'] text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-2 block">
            Fecha de Inicio
          </label>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
            ...
            <div className="bg-black/40 backdrop-blur-xl border border-white/5 p-6 relative overflow-hidden">
              <p className="font-['Inter'] text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-2">
                Utilidad Bruta (Ventas)
              </p>
              <h3 className="text-2xl font-black text-white font-['Inter']">
                {formatCurrency(grossProfit)}
              </h3>
              <div className="mt-2 text-[10px] text-zinc-400 flex justify-between">
                <span>
                  Ingresos: {formatCurrency(reportData.resumen.totalIngresos)}
                </span>
                <span>
                  Costos: -{formatCurrency(reportData.resumen.totalCostos)}
                </span>
              </div>
            ...
            {/* Resultado Neto */}
            <div
              className={`p-6 relative overflow-hidden shadow-2xl ${netProfit >= 0 ? "bg-white text-black" : "bg-zinc-900 text-white border border-zinc-800"}`}
            >
              <p
                className={`font-['Inter'] text-[10px] font-black uppercase tracking-[0.2em] mb-2 ${netProfit >= 0 ? "text-zinc-500" : "text-zinc-400"}`}
              >
                Resultado Neto Final
              </p>
              <h3 className="text-3xl font-black font-['Inter']">
                {formatCurrency(netProfit)}
              </h3>
              <p
                className={`text-[10px] font-bold mt-1 uppercase tracking-wider ${netProfit >= 0 ? "text-zinc-600" : "text-zinc-400"}`}
              >
                {netProfit >= 0
              ...
              <FiTrendingUp
                className={`absolute top-4 right-4 text-4xl opacity-10 ${netProfit >= 0 ? "text-black" : "text-white"}`}
              />
            </div>
            ...
            <div className="bg-black/40 backdrop-blur-xl border border-white/5 p-6 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-black text-white font-['Inter']">
                {reportData.resumen.margen.toFixed(2)}%
              </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
            ...
            <div className="bg-black/40 backdrop-blur-xl border border-white/5">
              <div className="p-6 border-b border-white/5 flex justify-between items-center">
                <h3 className="font-['Inter'] font-bold text-white uppercase text-sm tracking-wider flex items-center gap-2">
                  <FiActivity size={14} className="text-white" /> Gastos Fijos
                  Mensuales
                </h3>
                <span className="text-zinc-300 font-mono text-xs font-black">
                  -{formatCurrency(totalFixedExpenses)}
                </span>
              </div>
              <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
                {expensesData.fixed.length === 0 ? (
                  <p className="text-zinc-700 text-[10px] font-black uppercase tracking-widest text-center py-10 italic">
                  ...
                  <table className="w-full text-left text-[10px]">
                    <thead className="text-zinc-500 uppercase font-black bg-white/[0.02]">
                      <tr>
                        ...
                        <th className="py-3 text-right pr-4">Monto</th>
                      </tr>
                    </thead>
                      ...
                      {expensesData.fixed.map((item, i) => (
                        ...
                        >
                          <td className="py-3 pl-4 text-zinc-500">
                            {new Date(
                              item.vencimiento || item.createdAt,
                            ).toLocaleDateString()}
                          </td>
                          <td className="py-3 text-white uppercase font-bold group-hover:text-white transition-colors">
                            {item.nombre}
                          </td>
                          <td className="py-3 text-right text-zinc-300 font-black pr-4">
            ...
            <div className="bg-black/40 backdrop-blur-xl border border-white/5">
              <div className="p-6 border-b border-white/5 flex justify-between items-center">
                <h3 className="font-['Inter'] font-bold text-white uppercase text-sm tracking-wider flex items-center gap-2">
                  <FiMinusCircle size={14} className="text-white" /> Egresos
                  Variables de Caja
                </h3>
                <span className="text-zinc-300 font-mono text-xs font-black">
              ...
              <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
                {expensesData.variable.length === 0 ? (
                  <p className="text-zinc-700 text-[10px] font-black uppercase tracking-widest text-center py-10 italic">
                    Sin registros en este periodo
                  </p>
                ) : (
          ...
          <div className="flex justify-end mb-8">
            <button
              onClick={generatePDF}
              className="bg-white text-black font-['Inter'] font-black text-[10px] uppercase tracking-widest py-4 px-8 hover:bg-zinc-200 transition-all flex items-center gap-2 shadow-[0_10px_30px_rgba(255,255,255,0.1)]"
            >
          ...
          <div className="bg-black/40 backdrop-blur-xl border border-white/5">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
              <h3 className="font-['Inter'] font-bold text-white uppercase text-sm tracking-wider flex items-center gap-2">
                <FiActivity size={14} className="text-white" /> Desglose de
                Productos Vendidos
              </h3>
              <span className="bg-white/10 text-white border border-white/20 px-3 py-1 text-[9px] font-black tracking-widest uppercase">
                {reportData.productos.length} REGISTROS
              </span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.03]">
                    <th className="p-4 font-['Inter'] text-[9px] font-black text-zinc-500 uppercase tracking-widest">
                      Fecha
                    </th>
                    <th className="p-4 font-['Inter'] text-[9px] font-black text-zinc-500 uppercase tracking-widest">
                      Producto
                    ...
                    <th className="p-4 font-['Inter'] text-[9px] font-black text-zinc-500 uppercase tracking-widest text-right">
                      Monto Venta
                    </th>
                    <th className="p-4 font-['Inter'] text-[9px] font-black text-zinc-500 uppercase tracking-widest text-right">
                      CMV (Costo)
                    </th>
                    <th className="p-4 font-['Inter'] text-[9px] font-black text-white uppercase tracking-widest text-right bg-white/5">
                      Utilidad Neta
                    </th>
                  </tr>
                </thead>
                  ...
                  {reportData.productos.map((prod, idx) => (
                    ...
                    >
                      <td className="p-4 text-zinc-500">
                        {new Date(prod.fecha).toLocaleDateString()}
                      </td>
                      <td className="p-4">
                        <div className="text-white font-black uppercase group-hover:text-white transition-colors">
                          {prod.nombre}
                        </div>
                        <div className="text-[9px] text-zinc-600 mt-1 font-black flex items-center gap-1">
                          UNIDADES:{" "}
                          <span className="text-zinc-400 bg-white/5 px-1.5 rounded-sm">
                            {prod.cantidad}
                          </span>
                        </div>
                      </td>
                      <td className="p-4">
                        <span
                          className={`flex items-center gap-1.5 w-fit px-3 py-1 text-[8px] font-black tracking-widest border ${
                            prod.origen === "ECOMMERCE"
                      ...
                      <td className="p-4 text-right text-zinc-300 font-bold">
                        {formatCurrency(prod.precioVentaTotal)}
                      </td>
                      <td className="p-4 text-right text-zinc-500">
                        {formatCurrency(prod.costoTotal)}
                      </td>
                      <td className="p-4 text-right text-white font-black text-xs bg-white/[0.02]">
