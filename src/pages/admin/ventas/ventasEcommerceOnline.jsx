...
const styles = {
  glassCard:
    "bg-zinc-900/40 backdrop-blur-xl border border-zinc-800 shadow-2xl",
  heading: "font-['Inter'] font-[900] uppercase tracking-tighter leading-none",
  tech: "font-['Inter'] uppercase tracking-widest",
  buttonActive:
    "bg-white text-gray-900 font-['Inter'] font-[900] uppercase text-[10px] tracking-widest",
  buttonInactive:
    "text-gray-500 hover:text-white font-['Inter'] font-[900] uppercase text-[10px] tracking-widest",
  label: "text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]",
};
...
const PaqueteCard = ({ orden, onEstadoChange }) => {
  const estadoConfig =
    DISPATCH_ESTADOS[orden.dispatchStatus] || DISPATCH_ESTADOS.PENDIENTE;
  ...
  return (
    <div
      className={`${styles.glassCard} p-0 overflow-hidden transition-all hover:border-white font-['Inter']`}
    >
      {" "}
      {/* Header del paquete */}{" "}
      <div className="flex flex-col md:flex-row items-start justify-between p-5 border-b border-zinc-800/60 gap-4 md:gap-2">
        ...
        <div className="flex items-start gap-3">
          {" "}
          <div
            className={`p-2 rounded-none ${estadoConfig.bg} border ${estadoConfig.border} mt-0.5`}
          >
            {" "}
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              >
                ...
                PEDIDO #
                {orden.isGrouped
                  ? orden.originalIds.join(", #")
                  : orden.id}{" "}
              </span>{" "}
              {orden.isGrouped && (
                <span
                  className={`text-[8px] font-bold px-2 py-0.5 rounded-none border text-white bg-white/10 border-white/30`}
                >
                  {" "}
                  ENCOMIENDA{" "}
                </span>
              ...
              <span
                className={`text-[8px] font-bold px-2 py-0.5 rounded-none border ${isEcommerceNormal ? "text-zinc-300 bg-zinc-800 border-zinc-600" : "text-gray-500 bg-zinc-900 border-zinc-700"}`}
              >
                {" "}
            ...
            <p
              className={`${styles.tech} text-xs font-black text-white uppercase`}
            >
              {orden.name || "CLIENTE SIN NOMBRE"}
            </p>{" "}
            {orden.cellphone && (
        ...
        <div className="text-left md:text-right w-full md:w-auto pl-12 md:pl-0">
          {" "}
          <p className={`${styles.tech} text-lg font-black text-white`}>
      ...
      {orden.address && orden.address.trim() !== "" && (
        <div className="px-5 py-3 bg-black/30 border-b border-zinc-800/60 flex items-start gap-2">
          ...
          />{" "}
          <p className="text-[11px] text-gray-400 uppercase font-medium leading-relaxed">
            {" "}
            {[orden.address, orden.city, orden.province, orden.postalCode]
              .filter(Boolean)
              .join(", ")}{" "}
          </p>{" "}
        </div>
      ...
      {orden.shippingOption && orden.shippingOption.trim() !== "" && (
        <div className="px-5 py-2 border-b border-zinc-800/60 flex items-center gap-2">
          {" "}
          <FiTruck size={11} className="text-gray-500" />{" "}
          <span className="text-[10px] text-gray-400 font-bold">
            {orden.shippingOption}
          </span>{" "}
      ...
      <div className="p-5 space-y-2">
        ...
        {items.map((item, i) => (
          <div
            key={i}
            className="flex flex-wrap items-center justify-between text-[11px] py-1.5 border-b border-zinc-900/80 last:border-0 gap-x-4 gap-y-1"
          >
            {" "}
            <div className="flex items-center gap-2">
              ...
              {item.color && (
                <span className="text-gray-500 font-bold">• {item.color}</span>
              )}{" "}
          ...
          {Object.entries(DISPATCH_ESTADOS).map(([key, cfg], index) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center gap-1.5 py-2.5 text-[8px] font-black tracking-widest border transition-all ${styles.tech} ${isActive ? `${cfg.bg} ${cfg.color} ${cfg.border}` : "bg-transparent text-gray-600 border-zinc-800 hover:border-white hover:text-white"}`}
              >
                {" "}
...
const HistorialVentasOnline = () => {
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return pedidos.filter((p) => {
      ...
      const matchEstado =
        filtroEstado === "TODOS" || p.dispatchStatus === filtroEstado;
  ...
  const stats = useMemo(
    () => ({
      ...
      pendientes: pedidos.filter((p) => p.dispatchStatus === "PENDIENTE")
        .length,
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FE] text-gray-900 p-4 md:p-10 space-y-10 font-['Inter'] selection:bg-white selection:text-gray-900">
      {" "}
      {/* HEADER */}{" "}
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div>
          ...
          <p
            className={`${styles.tech} text-[10px] font-bold text-gray-600 mt-4 tracking-[0.4em]`}
          >
            {" "}
        ...
        */
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-3 bg-zinc-900 border border-zinc-800 hover:border-white transition-all text-gray-500 hover:text-white"
          >
            {" "}
            <FiRefreshCw
              size={16}
              className={cargando ? "animate-spin text-white" : ""}
            />{" "}
          </button>{" "}
          <div className="bg-zinc-900/40 border border-zinc-800 p-1 flex gap-1">
            ...
            {[
              ...
              <button
                ...
                className={`px-6 py-2.5 text-[10px] ${styles.tech} font-black transition-all flex items-center gap-2 ${tabActiva === id ? "bg-white text-gray-900" : "text-gray-500 hover:text-white"}`}
              >
                {" "}
                <Icon size={12} /> {label}{" "}
              </button>
        ...
        {[
          {
            ...
            value: formatearMoneda(stats.total),
            color: "text-white",
          },
          {
            label: "PENDIENTES",
          ...
          <div key={label} className={`${styles.glassCard} p-5`}>
            ...
            <p className={`${styles.tech} text-2xl font-black ${color}`}>
              {value}
            </p>{" "}
          </div>
        ))}{" "}
      ...
      {cargando ? (
        <div className="flex flex-col items-center justify-center py-32 text-zinc-700">
          ...
          />{" "}
          <span className={`${styles.tech} text-[11px] font-bold`}>
            CARGANDO VENTAS...
          </span>{" "}
        </div>
      ) : error ? (
        <div className="flex flex-col items-center justify-center py-32 text-gray-500">
          ...
          <span className={`${styles.tech} text-[11px] font-bold text-white`}>
            {error}
          </span>{" "}
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
              ...
              {["TODOS", "PENDIENTE", "EN_CAMINO", "RECIBIDO"].map((est) => (
                <button
                  ...
                  className={`px-4 py-2 text-[9px] ${styles.tech} font-black border transition-all ${filtroEstado === est ? "bg-white text-gray-900 border-white" : "bg-transparent text-gray-500 border-zinc-800 hover:border-white hover:text-white"}`}
                >
                  {" "}
                  {est.replace("_", " ")}{" "}
                </button>
          ...
          {pedidosFiltrados.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 opacity-20">
              ...
              <p className={`${styles.tech} text-[11px] font-bold`}>
        ...
        /* RANKING TAB */ <div className={`${styles.glassCard} p-8`}>
          {" "}
          <h3
            className={`${styles.heading} text-xl mb-8 text-white flex items-center gap-3`}
          >
            {" "}
          ...
          <div className="space-y-3">
            ...
            {productosMasVendidos.map((p, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-4 bg-zinc-900/60 border border-zinc-800 hover:border-white/30 transition-all group"
              >
                {" "}
                <div className="flex items-center gap-4">
                  {" "}
                  <span
                    className={`${styles.tech} text-2xl font-black text-zinc-700 group-hover:text-white transition-colors`}
                  >
                    {" "}
                    {String(i + 1).padStart(2, "0")}{" "}
                  </span>{" "}
                  <p className="text-sm font-bold uppercase tracking-tight font-['Inter']">
                    {p.nombre}
                  </p>{" "}
                </div>{" "}
                <div className="text-right">
                  ...
                  <p
                    className={`${styles.tech} text-[10px] font-bold text-gray-500 mt-1`}
                  >
            ...
            {productosMasVendidos.length === 0 && (
              <p className="text-center font-bold text-gray-600 py-12 uppercase font-['Inter'] text-sm tracking-widest">
