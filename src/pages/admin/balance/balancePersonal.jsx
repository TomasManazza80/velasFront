...
const PersonalBalanceModule = () => {
  ...
  const stats = useMemo(() => {
    ...
    const capital = safeBalance.reduce(
      (acc, curr) =>
        curr.tipo === "income"
          ? acc + parseFloat(curr.monto)
          : acc - parseFloat(curr.monto),
      0,
    );
    ...
    const deudaTotal = safeDebts.reduce(
      (acc, curr) =>
        acc + (parseFloat(curr.montoTotal) - parseFloat(curr.montoPagado)),
      0,
    );
  ...
  const handleDelete = async (type, id) => {
    ...
    let url;
    if (type === "balance")
      url = `${API.BALANCE}/eliminarBalancePersonal/${id}`;
  ...
  const handlePayInstallment = async (debt, installment) => {
    ...
    if (window.confirm("¿Registrar también como GASTO en balance personal?")) {
      try {
        ...
        console.error("AUTO_EXPENSE_ERROR", e);
      }
    }
  ...
  return (
    <div className="w-full space-y-6 md:space-y-10 pb-20 font-sans text-gray-500 p-4 md:p-0">
      {/* KPI DASHBOARD */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`${styles.card} p-6 md:p-8 border-l-4 border-gray-200`}>
          <p className={styles.tech}>Capital_Disponible</p>
          <h3 className={`${styles.title} text-2xl md:text-3xl mt-2`}>
            ${stats.capital.toLocaleString("es-AR")}
          </h3>
        </div>
        <div className={`${styles.card} p-6 md:p-8 border-l-4 border-red-500`}>
          <p className={styles.tech}>Gastos_Fijos_Pendientes</p>
          <h3
            className={`${styles.title} text-2xl md:text-3xl mt-2 text-red-500`}
          >
            ${stats.gastosPendientes.toLocaleString("es-AR")}
          </h3>
        </div>
        <div className={`${styles.card} p-6 md:p-8 border-l-4 border-zinc-500`}>
          <p className={styles.tech}>Deuda_Total_Amortizable</p>
          <h3 className={`${styles.title} text-2xl md:text-3xl mt-2`}>
            ${stats.deudaTotal.toLocaleString("es-AR")}
          </h3>
        </div>
      </div>

      {/* NAVEGACIÓN */}
      <div className="flex bg-white p-1.5 rounded-2xl border border-gray-200 w-full md:w-fit">
        {["balance", "deudas"].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 md:flex-none text-center px-4 md:px-8 py-3 rounded-xl text-xs font-black uppercase transition-all ${activeTab === tab ? "bg-white text-black shadow-lg shadow-white/20" : "hover:text-gray-900"}`}
          >
            {tab}
          </button>
        ))}
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className={`${styles.btnBw} hidden md:inline-flex`}
              >
                Nuevo_Movimiento
              </button>
            </div>
            <div className={styles.card}>
              ...
              <div className="md:hidden divide-y divide-gray-200">
                {balanceEntries.map((e) => (
                  <div
                    key={e.BalancePersonalId}
                    className="p-4 flex justify-between items-center gap-3"
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-gray-900 font-bold uppercase truncate text-sm">
                        {e.descripcion}
                      </p>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <p
                        className={`font-black text-sm text-right ${e.tipo === "income" ? "text-green-500" : "text-gray-900"}`}
                      >
                        {e.tipo === "income" ? "+" : "-"}$
                        {parseFloat(e.monto).toLocaleString("es-AR")}
                      </p>
                      <button
                        onClick={() => openModal("balance", e)}
                      ...
                      <button
                        onClick={() =>
                          handleDelete("balance", e.BalancePersonalId)
                        }
              ...
              <div className="hidden md:block overflow-x-auto custom-scrollbar">
                <table className="w-full text-left">
                  <thead className="bg-white text-xs font-black text-gray-900 uppercase tracking-widest">
                    <tr>
                    ...
                    {balanceEntries.map((e) => (
                      ...
                      >
                        <td className="p-6">
                          ...
                          <div className="text-xs opacity-40 uppercase tracking-tighter">
                            {e.fecha} | {e.cuenta}
                          </div>
                        </td>
                        <td
                          className={`p-6 text-right font-black ${e.tipo === "income" ? "text-green-500" : "text-gray-900"}`}
                        >
                          {e.tipo === "income" ? "+" : "-"}$
                          {parseFloat(e.monto).toLocaleString("es-AR")}
                        </td>
        ...
        {activeTab === "deudas" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {debtEntries.map((d) => {
              ...
              const perc =
                (parseFloat(d.montoPagado) / parseFloat(d.montoTotal)) * 100;
              return (
                <div key={d.DebtId} className={`${styles.card} p-6 md:p-8`}>
                  <p className={styles.tech}>{d.acreedor}</p>
                  <h5 className="text-lg font-black text-gray-900 uppercase mt-1">
                    {d.descripcion}
                  </h5>
                  <div className="my-6">
                    <div className="flex justify-between text-xs mb-2 font-black">
                      <span>AMORTIZADO</span>
                      <span className="text-gray-900">{perc.toFixed(1)}%</span>
                    </div>
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => openModal("deuda", d)}
                        className="p-2 text-gray-500 hover:text-gray-900"
                      >
                        <PencilSquareIcon className="w-5 h-5" />
                      </button>
                      <button
                        onClick={() => handleDelete("deuda", d.DebtId)}
                        className="p-2 text-gray-400 hover:text-red-600"
                      >
                        <TrashIcon className="w-5 h-5" />
                      </button>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200 grid grid-cols-4 gap-2">
                      {d.detalleCuotas.map((q) => (
                        <button
                          ...
                          className={`h-10 w-10 flex items-center justify-center rounded-lg text-xs font-black border transition-all ${
                            q.pagado
                              ? "bg-green-500/20 text-green-500 border-green-500/30"
                              : "bg-white text-gray-500 border-gray-200 hover:bg-white/20 hover:text-gray-900 hover:border-gray-2000"
                          }`}
                        >
                          C{q.numero}
                        </button>
        ...
        {modal.open && (
          ...
          >
            <motion.div
              initial={{ scale: 0.95 }}
              animate={{ scale: 1 }}
              className={`${styles.card} w-full max-w-2xl p-6 md:p-12`}
            >
              <div className="flex justify-between items-center mb-10">
                <h3 className={styles.title}>
                  {modal.data ? "Actualizar_Registro" : "Nueva_Entrada"} |{" "}
                  {modal.type}
                </h3>
                <button onClick={closeModal}>
                  <XMarkIcon className="w-8 h-8 text-gray-500 hover:text-gray-900" />
                </button>
              </div>
              <form
                onSubmit={handleSave}
              ...
              >
                {modal.type === "deuda" && (
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, acreedor: e.target.value })
                      }
                      required
                    />
                    <input
                ...
                <button
                  type="submit"
                  className={`${styles.btnBw} md:col-span-2 mt-4`}
                >
      ...
      <div className="md:hidden fixed bottom-6 right-4 z-[110]">
        <button
          onClick={() =>
            openModal(activeTab === "balance" ? "balance" : "deuda")
          }
