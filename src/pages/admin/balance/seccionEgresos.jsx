...
const EgressModule = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resEgress.ok && resBalance.ok) {
        ...
        if (resEgress.ok && !resBalance.ok) {
          errorMessage =
            "ATENCIÓN: El egreso se guardó, pero falló la actualización del balance. Por favor, revise manualmente para evitar descuadres.";
          console.error(
            "FEDECELL_INCONSISTENCY: Egress created, but Balance update failed.",
            { egressResponse: resEgress, balanceResponse: resBalance },
          );
        }
        alert(errorMessage);
        setIsSubmitting(false);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error("FEDECELL_DELETE_ERROR:", e);
    }
  };
  ...
  const filtered = dataInRange.filter(
    (e) =>
  ...
  return (
    <div
      className={`w-full space-y-12 pb-20 ${styles.body} animate-in fade-in duration-1000`}
    >
      {/* DASHBOARD */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className={styles.statCard}>
          ...
          <h4
            className={`${styles.title} text-3xl md:text-4xl mt-4 text-gray-900`}
          >
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-8 border-b border-gray-200 pb-10">
        <div className="flex bg-white p-1.5 rounded-2xl border border-gray-200 w-full lg:w-auto">
          <button
            onClick={() => setView("list")}
            className={`flex-1 lg:flex-none px-10 py-4 rounded-xl text-sm font-sans font-black transition-all ${view === "list" ? "bg-white text-black shadow-lg shadow-white/20" : "text-gray-500 hover:text-gray-900"}`}
          >
            DATA_LISTADO
          </button>
          <button
            onClick={() => setView("form")}
            className={`flex-1 lg:flex-none px-10 py-4 rounded-xl text-sm font-sans font-black transition-all ${view === "form" ? "bg-white text-black shadow-lg shadow-white/20" : "text-gray-500 hover:text-gray-900"}`}
          >
            NUEVA EXTRACCIÓN
          </button>
        </div>
        <div className="w-full lg:w-auto flex flex-col md:flex-row gap-4">
          <div className="relative w-full lg:w-[450px]">
            <MagnifyingGlassIcon className="w-5 h-5 absolute left-5 top-1/2 -translate-y-1/2 text-gray-900" />
            <input
              type="text"
              placeholder="BUSCAR_EGRESO..."
              className={`${styles.input} pl-14 h-16 bg-white uppercase font-sans text-xs`}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl p-2 h-16 w-full md:w-auto">
      ...
      <div className="min-h-[500px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 space-y-4">
            <ArrowPathIcon className="w-12 h-12 text-gray-900 animate-spin opacity-20" />
            <p className={styles.tech}>CONECTANDO_AL_SERVIDOR...</p>
          </div>
        ) : view === "list" ? (
          <div
            className={`${styles.glass} overflow-x-auto custom-scrollbar animate-in fade-in slide-in-from-top-4 duration-700`}
          >
            <table className="w-full text-left">
              <thead>
                <tr className="bg-white/[0.03]">
                  ...
                  <th className={`px-10 py-7 ${styles.tech}`}>
                    DETALLE_OPERATIVO
                  </th>
                  <th className="px-10 py-7 text-right font-sans text-xs font-bold text-gray-500 tracking-widest uppercase">
                    ELIMINAR
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {filtered.map((eg) => (
                  <tr
                    key={eg.id || eg.EgressId}
                    className="hover:bg-gray-50 transition-colors group font-sans"
                  >
                    ...
                    <td className="px-10 py-8">
                      <span className="text-xs font-black py-2 px-3 bg-white border border-gray-200 text-gray-500 rounded-lg font-sans uppercase">
                        {medioLabels[eg.medio] || eg.medio}
                      </span>
                    </td>
                    <td className="px-10 py-8 text-sm font-medium text-gray-700 uppercase">
                      {eg.responsable || "-"}
                    </td>
                    <td className="px-10 py-8 text-sm text-gray-500 whitespace-nowrap">
                      {new Date(eg.createdAt || eg.fecha).toLocaleString(
                        "es-AR",
                        {
                          day: "2-digit",
                          month: "2-digit",
                    ...
                    <td className="px-10 py-8 text-sm font-bold text-gray-900 uppercase tracking-wider max-w-xs truncate">
                      {eg.detalle}
                    </td>
          ...
          <div className="max-w-4xl mx-auto py-10 animate-in slide-in-from-bottom-8 duration-500 relative">
            <AnimatePresence>
              {showSuccess && (
                ...
                >
                  <CheckCircleIcon className="w-24 h-24 text-gray-900" />
                  <h3 className={`${styles.title} text-2xl mt-4`}>
                    EXTRACCIÓN EXITOSA
                  </h3>
            ...
            <div className={`${styles.glass} p-6 md:p-20`}>
              <form className="space-y-12" onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-4">
                    ...
                    <input
                      ...
                      className={`${styles.input} h-20 text-3xl font-sans font-black`}
                      placeholder="0.00"
                    />
                  </div>
                    ...
                    <AnimatePresence>
                      {isSuggestionsVisible && (
                        <motion.div
                          ...
                          exit={{ opacity: 0, y: -5 }}
                          className="absolute z-50 w-full mt-1 bg-white border border-gray-200 shadow-xl rounded-xl max-h-48 overflow-y-auto custom-scrollbar"
                        >
                          {responsablesList
                            .filter(
                              (r) =>
                  ...
                  <div className="space-y-4">
                    ...
                    <select
                      className={`${styles.input} h-20 bg-zinc-900 font-sans font-black text-xs tracking-widest uppercase cursor-pointer`}
                      value={medio}
                  ...
                  <div className="space-y-4">
                    <label className={styles.tech}>
                      DESCRIPCIÓN_DE_OPERACIÓN
                    </label>
                    <textarea
                      rows="2"
                      className={`${styles.input} h-20 resize-none uppercase text-xs tracking-widest leading-relaxed pt-6`}
                      value={detalle}
                ...
                <button
                  ...
                  className={`w-full ${styles.btnBw} h-24 text-sm tracking-[0.5em] disabled:bg-zinc-700 disabled:cursor-not-allowed`}
                >
