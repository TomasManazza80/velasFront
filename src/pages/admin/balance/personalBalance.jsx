...
const PersonalBalance = () => {
  ...
  const handleCreateTransaction = async (e) => {
    ...
    try {
      ...
      /* Reset form setDescripcion(''); setMonto(''); setCategoria(''); setMedio('efectivo'); setTipo('expense'); fetchTransactions(); setView('list'); */
    } catch (err) {
  ...
  const handlePayInstallment = async (debt, installment) => {
    ...
    if (
      !window.confirm(
        `¿Pagar cuota #${installment.numero} de $${parseFloat(installment.monto).toLocaleString()}?`,
      )
    )
      return;
    ...
    const updatedDetails = debt.detalleCuotas.map((d) =>
      d.numero === installment.numero
        ? { ...d, pagado: true, fechaPago: new Date().toISOString() }
        : d,
    );
    try {
      await axios.put(`${API_DEBT_URL}/actualizarDeuda/${debt.DebtId}`, {
        montoPagado:
          parseFloat(debt.montoPagado) + parseFloat(installment.monto),
        estado:
          parseFloat(debt.montoPagado) + parseFloat(installment.monto) >=
          parseFloat(debt.montoTotal) - 1
            ? "pagado"
            : "pendiente",
  ...
  return (
    <div
      className={` min-h-screen font-sans font-medium text-gray-900/50 animate-in fade-in duration-700`}
    >
      {" "}
      {/* HEADER & SUMMARY */}{" "}
      <header className="mb-12">
        ...
        <div className="flex justify-between items-end mb-10">
          ...
          <div>
            {" "}
            <h2 className={styles.title + " text-5xl md:text-6xl"}>
              BALANCE_<span className="text-[#FF8C00]">PERSONAL</span>
            </h2>{" "}
            <div className="h-1 w-24 bg-[#FF8C00] mt-4 shadow-[0_0_20px_rgba(255,140,0,0.6)]"></div>{" "}
            <p className={styles.tech + " mt-4"}>
              Private_Ledger /* User_01
            </p>{" "}
          </div>{" "}
          */
          <button
        ...
        {view === "list" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {" "}
            <div className="bg-gray-50 border border-gray-200 p-6 backdrop-blur-xl">
              {" "}
              <p className={styles.tech + " text-gray-500 mb-2"}>
                NET_AVAILABLE_ASSETS
              </p>{" "}
              <h3
                className={`font-['Montserrat'] font-black text-4xl ${totals.balance >= 0 ? "text-gray-900" : "text-red-500"}`}
              >
                {" "}
            ...
            <div className="bg-gray-50 border border-gray-200 p-6 backdrop-blur-xl">
              ...
              <p
                className={
                  styles.tech + " text-gray-500 mb-2 flex items-center gap-2"
                }
              >
                <FiArrowUp className="text-green-500" /> TOTAL_INCOME
              </p>{" "}
            ...
            <div className="bg-gray-50 border border-gray-200 p-6 backdrop-blur-xl">
              ...
              >
                <FiArrowDown className="text-red-500" /> TOTAL_EXPENSES
              </p>{" "}
      ...
      {view === "list" && (
        <section className="mb-20">
          {" "}
          <div className="flex items-center justify-between gap-4 mb-8 border-b border-gray-200 pb-4">
            {" "}
            ...
            <button
              ...
              className={`px-6 py-2 text-xs font-black uppercase tracking-widest border ${showDebtForm ? "border-red-500 text-red-500" : "border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-gray-900"} transition-all`}
            >
              {" "}
          ...
          {showDebtForm && (
            <form
              ...
              className="mb-12 bg-gray-50 p-8 border border-orange-500/20 animate-in fade-in slide-in-from-top-4"
            >
              {" "}
              <h4 className={styles.tech + " mb-6 text-orange-500"}>
                REGISTRAR_PASIVO_FINANCIERO
              </h4>{" "}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <input
                  ...
                  required
                  value={debtQuotas}
                  onChange={(e) => setDebtQuotas(e.target.value)}
                  className={styles.input}
                />{" "}
              </div>{" "}
              <button
                type="submit"
                className="w-full py-3 bg-orange-600 hover:bg-orange-500 text-gray-900 font-black text-xs uppercase tracking-widest"
              >
                CONFIRMAR_DEUDA
              </button>{" "}
            </form>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            {debts.map((debt) => {
              const progress =
                (parseFloat(debt.montoPagado) / parseFloat(debt.montoTotal)) *
                100;
              const isPaid = progress >= 99.9;
              return (
                <div
                  key={debt.DebtId}
                  className={`relative p-8 border ${isPaid ? "border-green-500/20 bg-green-500/[0.02]" : "border-gray-200 bg-gray-50"} backdrop-blur-xl transition-all`}
                >
                  {" "}
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div>
                      {" "}
                      <h4 className="font-['Montserrat'] font-black text-gray-900 text-xl leading-tight uppercase">
                        {debt.descripcion}
                      </h4>{" "}
                      <p className={styles.tech + " mt-2 text-gray-500"}>
                        ACREEDOR: {debt.acreedor}
                        /* PLAN: {debt.cuotasTotales} CUOTAS
                      </p>{" "}
                    </div>{" "}
                    */
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-black text-gray-900">
                        ${parseFloat(debt.montoTotal).toLocaleString()}
                      </p>{" "}
                      <p className={styles.tech + " text-gray-600"}>
                        RESTANTE: $
                        {(
                          parseFloat(debt.montoTotal) -
                          parseFloat(debt.montoPagado)
                        ).toLocaleString()}
                      </p>{" "}
                    </div>{" "}
                  </div>{" "}
                  ...
                  {debt.detalleCuotas && Array.isArray(debt.detalleCuotas) ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                      ...
                      {debt.detalleCuotas.map((quota) => (
                        <button
                          ...
                          className={`p-3 border text-left transition-all relative group ${quota.pagado ? "border-green-500/30 bg-green-50 text-green-500 cursor-default" : "border-gray-200 bg-black hover:border-orange-500 hover:bg-orange-500/10 cursor-pointer"} `}
                        >
                          {" "}
                          ...
                          <p className="font-bold text-sm mb-1">
                            ${parseFloat(quota.monto).toLocaleString()}
                          </p>{" "}
                          {quota.pagado ? (
                            <FiCheckCircle
                            ...
                            <span className="text-[8px] text-orange-500 font-black uppercase opacity-0 group-hover:opacity-100 transition-opacity">
                              PAGAR &rarr;
                            </span>
                          )}{" "}
                        </button>
                    ...
                    !isPaid && (
                      <div className="p-4 border border-gray-200 bg-gray-50 text-center">
                        {" "}
                        <p className={styles.tech + " text-gray-500 mb-2"}>
                          REGISTRO SIN DETALLE DE CUOTAS
                        </p>{" "}
                        <button
                          onClick={async () => {
                            const remaining =
                            ...
                            const amountStr = prompt(
                              `Ingrese monto a pagar (Restante: $${remaining.toLocaleString()})`,
                            );
                            if (!amountStr) return;
                          ...
                          className="px-6 py-2 border border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-gray-900 text-xs font-black uppercase"
                        >
                          {" "}
                          PAGAR SALDO{" "}
                        </button>{" "}
      ...
      {view === "list" && (
        <div className={styles.glass}>
          ...
          <table className="w-full text-left">
            {" "}
            <thead className="bg-white/[0.03] border-b border-gray-200 font-['JetBrains_Mono'] text-[#FF8C00] text-xs uppercase tracking-[0.2em]">
              {" "}
            ...
            <tbody className="divide-y divide-gray-200 font-['JetBrains_Mono'] text-sm">
              {" "}
              {transactions.map((t) => (
                <tr
                  key={t.PersonalBalanceId}
                ...
                >
                  ...
                  <td className="p-6">
                    ...
                    <p className="text-gray-600 font-bold mt-1 text-xs uppercase">
                      {t.categoria}
                    </p>{" "}
                  </td>{" "}
                  <td className="p-6">
                    {" "}
                    <p
                      className={`font-black text-[13px] ${t.tipo === "income" ? "text-green-500" : "text-red-500"}`}
                    >
                      {" "}
                      {t.tipo === "income" ? "+" : "-"}$
                  ...
                  <td className="p-6">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-900 opacity-80 uppercase tracking-wider">
                        {medioLabels[t.cuenta] || t.cuenta}
                      </span>{" "}
                    </div>{" "}
                  </td>{" "}
                  <td className="p-6">
                  ...
                  <td className="p-6 text-right">
                    ...
                    <button
                      ...
                      className="p-3 text-zinc-700 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"
                    >
                      {" "}
              ...
              {transactions.length === 0 && (
                <tr>
                  ...
                  <td
                    colSpan="6"
                    className="p-12 text-center text-gray-600 italic"
                  >
                    {" "}
      ...
      {view === "form" && (
        <div
          className={
            styles.glass +
            " p-12 md:p-20 animate-in slide-in-from-right-10 fade-in duration-500"
          }
        >
          {" "}
          <form
          ...
          >
            ...
            <div className="border-l-4 border-orange-500 pl-8">
              {" "}
              <h3 className={styles.title + " text-4xl mb-2"}>
                NUEVO_REGISTRO
              </h3>{" "}
              <p className={styles.tech}>ADD_TRANSACTION_TO_LEDGER</p>{" "}
            </div>{" "}
            <div className="flex gap-4 p-1 bg-white w-fit mb-8">
              {" "}
              <button
                type="button"
                onClick={() => setTipo("income")}
                className={`px-8 py-3 font-black text-xs uppercase tracking-widest transition-all ${tipo === "income" ? "bg-green-600 text-gray-900 shadow-lg shadow-green-900/50" : "text-gray-500 hover:text-gray-900"}`}
              >
                {" "}
                Ingreso{" "}
              </button>{" "}
              <button
                ...
                className={`px-8 py-3 font-black text-xs uppercase tracking-widest transition-all ${tipo === "expense" ? "bg-red-600 text-gray-900 shadow-lg shadow-red-900/50" : "text-gray-500 hover:text-gray-900"}`}
              >
                {" "}
                Gasto{" "}
              </button>{" "}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-4">
                ...
                <input
                  ...
                  className={
                    ...
                    (tipo === "income" ? "text-green-500" : "text-red-500")
                  }
                  placeholder="0.00"
                />{" "}
              </div>{" "}
              ...
              <div className="space-y-4">
                ...
                <select
                  ...
                  className={
                    styles.input +
                    " h-16 bg-zinc-900 font-['Montserrat'] font-black text-xs tracking-widest uppercase cursor-pointer"
                  }
                >
                  {" "}
            ...
            <button
              type="submit"
              className={`w-full h-24 text-sm tracking-[0.5em] font-['Montserrat'] font-black uppercase transition-all hover:brightness-110 active:scale-[0.99] ${tipo === "income" ? "bg-green-600 text-gray-900 shadow-[0_0_30px_rgba(22,163,74,0.3)]" : "bg-red-600 text-gray-900 shadow-[0_0_30px_rgba(220,38,38,0.3)]"} `}
            >
              {" "}
