  ...
  if (hexMatch) {
    ...
    hexMatch.forEach((hex) => {
      const found = PREDEFINED_COLORS.find(
        (c) => c.code.toLowerCase() === hex.toLowerCase(),
      );
...
const PaymentsSection = ({
  ...
  const fetchBalance = async () => {
    ...
    try {
      ...
      const combinedData = [
        ...(Array.isArray(resBalance) ? resBalance : []),
        ...ecommerceData,
      ].sort((a, b) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/BorraBalanceMensual/${id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const stats = useMemo(() => {
    ...
    const totalByMethod = dataInRange.reduce((acc, curr) => {
      ...
      if (method === "mixto" && curr.detalles_pago?.mixto) {
        ...
        if (mixto.efectivo)
          acc["efectivo"] =
            (acc["efectivo"] || 0) + (parseFloat(mixto.efectivo) || 0);
        if (mixto.transferencia)
          acc["transferencia"] =
            (acc["transferencia"] || 0) +
            (parseFloat(mixto.transferencia) || 0);
        if (mixto.debito)
          acc["debito"] =
            (acc["debito"] || 0) + (parseFloat(mixto.debito) || 0);
  ...
  const filteredEntries = dataInRange.filter((item) => {
    ...
    const matchesSearch2 = (item.cliente || "")
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesCategory =
      selectedCategory === "EXTRACCIONES"
        ? parseFloat(item.monto) < 0
        : selectedCategory
          ? item.metodo_pago === selectedCategory
          : true;
  ...
  return (
    <div
      className={`w-full space-y-6 md:space-y-10 py-6 px-4 md:px-0 ${styles.body}`}
    >
      {/* 1. DASHBOARD DE CABECERA (Estético y Claro) */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
        <div className={`${styles.statCard} p-4 md:p-6 overflow-hidden`}>
          <p className="text-xs md:text-xs tracking-widest text-gray-500 uppercase mb-2 break-words">
            Balance Neto Rango
          </p>
          <h4
            className={`text-lg md:text-2xl xl:text-3xl break-words ${stats.total >= 0 ? "text-green-500" : "text-red-500"}`}
          >
        ...
        <div className={`${styles.statCard} p-4 md:p-6 overflow-hidden`}>
          ...
          <h4 className="text-lg md:text-2xl xl:text-3xl break-words text-gray-900">
            ${stats.hoy.toLocaleString("es-AR")}
          </h4>
        </div>
        <div
          className={`${styles.statCard} border-red-200 p-4 md:p-6 overflow-hidden`}
        >
          <p className="text-xs md:text-xs tracking-widest text-red-500 uppercase mb-2 break-words">
            Extracciones en Rango
          </p>
          <h4 className="text-lg md:text-2xl xl:text-3xl break-words text-red-500">
            -${stats.totalExtracciones.toLocaleString("es-AR")}
          </h4>
        </div>
        <div className={`${styles.statCard} p-4 md:p-6 overflow-hidden`}>
          <p className="text-xs md:text-xs tracking-widest text-gray-500 uppercase mb-2 break-words">
          ...
          <h4 className="text-lg md:text-2xl xl:text-3xl break-words text-gray-900">
            {dataInRange.length}{" "}
            <span className="text-xs md:text-xs text-gray-500 font-mono">
              OP_UNITS
            </span>
          </h4>
        </div>
        ...
        <div className="w-full xl:w-1/4 space-y-4 md:space-y-6">
          <div className={`${styles.glass} p-4 md:p-6`}>
            <h2 className={`${styles.title} text-xs mb-6 flex items-center`}>
              <Square3Stack3DIcon className="w-4 h-4 mr-2 text-gray-900" />
              Filtrar Por Método
            </h2>
            <div className="flex flex-row md:flex-col gap-2 md:gap-y-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0 custom-scroll md:max-h-[500px] md:overflow-y-auto md:pr-2 no-scrollbar-mobile">
              ...
              <button
                onClick={() =>
                  setSelectedCategory(
                    selectedCategory === "EXTRACCIONES" ? null : "EXTRACCIONES",
                  )
                }
                className={`shrink-0 md:w-full flex justify-between items-center h-10 md:h-12 px-4 md:p-4 rounded-lg text-xs transition-all border whitespace-nowrap ${
                  selectedCategory === "EXTRACCIONES"
                    ? "bg-red-600 text-gray-900 border-red-600 shadow-lg "
                    : "bg-red-50 text-red-500 border-red-200 hover:border-red-500"
                }`}
              >
                <span className="font-black uppercase tracking-widest">
                  VER EXTRACCIONES
                </span>
                <span className="font-mono font-bold ml-2 md:ml-0">
                  -${stats.totalExtracciones.toLocaleString("es-AR")}
                </span>
              </button>

              <div className="hidden md:block h-px bg-white my-4"></div>
                ...
                <button
                  ...
                  className={`shrink-0 md:w-full flex justify-between items-center h-10 md:h-12 px-4 md:p-4 rounded-lg text-xs transition-all border whitespace-nowrap ${
                    selectedCategory === key
                      ...
                      : "bg-white text-gray-500 border-gray-200 hover:border-gray-2000"
                  }`}
                >
                  ...
                  <span
                    className={`${styles.tech} ml-2 md:ml-0 ${selectedCategory === key ? " text-black" : ""}`}
                  >
        ...
        <div className="w-full xl:w-3/4 space-y-6">
          <div className="flex flex-col md:flex-row gap-3 md:gap-4 items-center">
            <div className="relative group flex-1 w-full">
              <MagnifyingGlassIcon className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-900" />
              <input
                type="text"
                placeholder="BUSCAR OPERACIÓN POR PRODUCTO..."
                value={searchTerm} // Aseguramos que el valor esté controlado
                onChange={(e) => setSearchTerm(e.target.value)}
                className={`${styles.input} pl-12 h-12 md:h-14 bg-white tracking-widest uppercase`}
              />
            </div>
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg p-2 h-12 md:h-14 w-full md:w-auto">
              ...
              <input
                ...
                className="bg-white text-gray-900 focus:outline-none text-sm w-full md:w-32"
            ...
            <button
              ...
              className="w-full md:w-auto p-3 md:p-4 bg-red-50 rounded-lg border border-red-200 hover:border-red-500 hover:bg-red-500/20 transition-all flex justify-center items-center text-red-500 text-xs uppercase font-black tracking-widest"
            ...
            <button
              ...
              className="w-full md:w-auto p-3 md:p-4 bg-white rounded-lg border border-gray-200 hover:border-gray-200 transition-all flex justify-center items-center"
            >
              <ArrowPathIcon
          ...
          <div className={styles.glass}>
              ...
              <AnimatePresence>
                {Object.values(
                  filteredEntries.reduce((acc, item) => {
                    ...
                    const id =
                      item.id_transaccion || `LEGACY_${item.BalanceMensualId}`;
                    if (!acc[id]) {
                      ...
                      acc[id] = {
                        ...
                        hora: timeSource
                          ? new Date(timeSource).toLocaleTimeString("es-AR", {
                              hour: "2-digit",
                              minute: "2-digit",
                            })
                          : "--:--",
                  ...
                  .map((group) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isEgress && (
                          <div className="p-4 flex items-center justify-between bg-red-50 border-l-4 border-red-500">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500 font-mono mt-1">
                                  ...
                                  • {group.hora} •{" "}
                                  {group.items[0]?.metodo_pago?.toUpperCase()}
                                </p>
                              </div>
                            </div>
                                ...
                                <button
                                  onClick={() =>
                                    group.items.forEach((i) =>
                                      handleDelete(i.BalanceMensualId || i.id),
                                    )
                                  }
                        ...
                        {isGenericIncome && (
                          <div className="p-4 flex items-center justify-between bg-green-50 border-l-4 border-green-500">
                            <div className="flex items-center gap-4">
                        ...
                        {!isEgress && !isGenericIncome && (
                          <div className="p-4 flex items-center justify-between bg-white cursor-pointer group">
                            {(() => {
                              const itemsByProduct = group.items.reduce(
                                (acc, item) => {
                                  const productName =
                                  ...
                                  acc[productName].totalAmount +=
                                    parseFloat(item.monto) || 0;
                                  acc[productName].ids.push(
                                  ...
                                  <summary className="flex flex-col md:flex-row items-start md:items-center justify-between w-full list-none py-2 md:py-0">
                                    ...
                                    <div className="flex flex-col md:hidden w-full gap-1">
                                      <div className="flex justify-between items-center mb-1">
                                        <span className="text-gray-900 font-black text-sm uppercase truncate max-w-[60%]">
                                          {group.cliente}
                                        </span>
                                        <span
                                          className={`text-lg font-black tracking-tighter ${group.total >= 0 ? "text-green-500" : "text-red-500"}`}
                                        >
                                          ${group.total.toLocaleString("es-AR")}
                                        </span>
                                      </div>
                                      <div className="flex justify-between items-center text-gray-500 text-xs">
                                        ...
                                        <span className="text-gray-900 font-black text-xs uppercase tracking-tighter">
                                          {originLabels[group.origen] ||
                                            group.origen.toUpperCase()}
                                        </span>
                                      </div>
                                    </div>
                                    ...
                                    <div className="hidden md:flex items-center space-x-4">
                                      <span
                                        className={`text-xl font-black tracking-tighter ${group.total >= 0 ? "text-green-500" : "text-red-500"}`}
                                      >
                                        ${group.total.toLocaleString("es-AR")}
                                      </span>
                                      <Square3Stack3DIcon className="w-5 h-5 text-gray-500 group-hover:text-gray-900 transition-colors" />
                                    </div>
                                  </summary>
                                  ...
                                  <div className="mt-4 pt-4 border-t border-gray-200">
                                    ...
                                    <div className="md:hidden divide-y divide-white/5">
                                      {consolidatedItems.map((item) => (
                                        <div
                                          key={item.ids.join("-")}
                                          className="p-3 flex justify-between items-center gap-2 hover:bg-white transition-colors cursor-help"
                                          onDoubleClick={() =>
                                            setInspectedProduct(item.producto)
                                          }
                                          title="Doble clic para ver detalles del producto"
                                        >
                                          <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium text-gray-700 truncate">
                                              {getColorName(item.producto)}
                                            </p>
                                            <p className="text-xs text-gray-500 uppercase">
                                              {item.marca || "---"} |{" "}
                                              {item.categoria || "---"}
                                            </p>
                                            <div className="text-xs text-gray-500 uppercase mt-2 space-y-1">
                                              {item.payments.map((p, i) => {
                                                const bankInfo =
                                                  p.details && p.details.banco
                                                    ? ` (${p.details.banco})`
                                                    : "";
                                                const cardInfo =
                                                  p.tarjeta_digitos
                                                ...
                                                return (
                                                  ...
                                                  >
                                                    ...
                                                    <span className="text-gray-900 font-mono whitespace-nowrap">
                                                      $
                                          ...
                                          <div className="flex items-center gap-2 shrink-0">
                                            {item.BalanceMensualId && (
                                              <button
                                                ...
                                                className="p-2 text-gray-500 hover:text-gray-900 transition-colors h-10 w-10 flex items-center justify-center"
                                            ...
                                            <button
                                              ...
                                              className="p-2 text-gray-500 hover:text-red-500 transition-colors h-10 w-10 flex items-center justify-center"
                                    ...
                                    <div className="hidden md:block overflow-x-auto custom-scrollbar">
                                      <table className="w-full text-left">
                                        <thead className="opacity-50 text-xs uppercase text-gray-500">
                                          <tr>
                                        ...
                                        <tbody>
                                          {consolidatedItems.map((item) => (
                                            <tr
                                              key={item.ids.join("-")}
                                              className="border-b border-gray-200 last:border-0 hover:bg-white transition-colors cursor-help"
                                              onDoubleClick={() =>
                                                setInspectedProduct(
                                                  item.producto,
                                                )
                                            ...
                                            >
                                              ...
                                              <td className="py-3 text-xs text-gray-500 uppercase">
                                                {item.categoria || "---"}
                                              </td>
                                              <td className="py-3 text-xs text-gray-500 font-black">
                                                {originLabels[
                                                  item.origenDeVenta
                                                ] || "S/D"}
                                              </td>
                                              ...
                                              <td
                                                className={`py-3 text-sm font-mono text-right align-top ${item.totalAmount < 0 ? "text-red-500" : "text-green-500"}`}
                                              ></td>
            ...
            {filteredEntries.length === 0 && !loading && (
              <div className="p-24 text-center">
                ...
                <p
                  className={`${styles.tech} text-xs uppercase opacity-30 tracking-[0.3em]`}
                >
                  Sin Sincronizaciones Encontradas
                </p>
              </div>
            )}
          </div>
