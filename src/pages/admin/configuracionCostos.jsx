...
const STYLES = {
  ...
  label:
    "font-['Inter'] font-medium text-[10px] text-zinc-500 uppercase tracking-[0.2em] mb-2 block",
  tech: "font-['JetBrains_Mono'] tracking-widest uppercase",
  input:
    "w-full bg-black border border-zinc-800 rounded-none py-3 px-4 text-sm text-white font-['JetBrains_Mono'] focus:border-[#ff8c00] focus:ring-1 focus:ring-[#ff8c00] outline-none transition-all placeholder:text-zinc-700",
  glass: "bg-white/[0.02] backdrop-blur-xl border border-white/10 shadow-2xl",
  buttonPrimary:
    "bg-[#ff8c00] text-black font-['Montserrat'] font-[900] uppercase tracking-widest py-4 px-8 rounded-none hover:bg-white transition-all shadow-[0_10px_30px_rgba(255,140,0,0.2)]",
  buttonSecondary:
    "bg-zinc-900 text-white font-['Montserrat'] font-[900] uppercase tracking-widest py-4 px-8 rounded-none hover:bg-zinc-800 transition-all border border-zinc-800",
};
...
const ConfiguracionCostos = () => {
      ...
      try {
        ...
        if (shipRes.data && shipRes.data.length > 0) {
          ...
          shipRes.data.forEach((r) => {
            if (newShipping.hasOwnProperty(r.zona))
              newShipping[r.zona] = r.costo;
          });
        ...
        if (globalRes.data) {
          ...
          const globalIncAttr = globalRes.data.find(
            (c) => c.key === "global_increase",
          );
        ...
        setErrorLocal(
          "ERROR_DE_SINCRONIZACIÓN: NO SE PUDO CONECTAR CON EL SERVIDOR",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleUpdateVariantPrice = async (prodId, variantIdx, field, value) => {
    ...
    try {
      ...
      setSearchResults((prev) =>
        prev.map((p) =>
          p.id === prodId ? { ...p, variantes: updatedVariantes } : p,
        ),
      );
    } catch (err) {
  ...
  const handleAumentoChange = (e) => {
    ...
    setNuevoAumento((prev) => ({
      ...prev,
      [name]:
        name === "cuotas" || name === "porcentajeAumento"
          ? parseFloat(value)
          : value.toUpperCase(),
    }));
  };
  ...
  const handleAddAumento = (e) => {
    ...
    setErrorLocal(null);
    if (
      !nuevoAumento.banco.trim() ||
      nuevoAumento.cuotas <= 0 ||
      nuevoAumento.porcentajeAumento < 0
    ) {
      setErrorLocal("ERROR_LOG: CAMPOS_INVÁLIDOS_EN_RECARGO_TARJETA");
      return;
    }
    setAumentosPorTarjeta((prev) => [
      ...prev,
      { ...nuevoAumento, id: Date.now() },
    ]);
  ...
  const handleEditSave = (e) => {
    ...
    setErrorLocal(null);
    if (
      !reglaAEditar.banco.trim() ||
      reglaAEditar.cuotas <= 0 ||
      reglaAEditar.porcentajeAumento < 0
    ) {
    ...
    setAumentosPorTarjeta((prev) =>
      prev.map((regla) =>
        regla.id === reglaAEditar.id ? reglaAEditar : regla,
      ),
    );
  ...
  const handleSaveConfig = async () => {
    ...
    try {
        ...
        configs: [
          {
            ...
            description: "Porcentaje de IVA global",
          },
          {
            key: "global_increase",
            value: configGlobal.aumentoGlobalProductos,
            description: "Aumento global de precios",
          },
          {
            key: "mp_fee",
            value: configGlobal.mpFee,
            description: "Comisión de Mercado Pago",
          },
        ],
      });

      // Guardar Costos de Envío
      const shippingPromises = Object.entries(costosEnvio).map(
        ([zona, costo]) =>
      ...
      const bankPromises = aumentosPorTarjeta
        .filter((a) => !a.createdAt)
        .map((a) => axios.post(`${API_BASE}/gastos/bank-rates`, a));

      await Promise.all([...shippingPromises, ...bankPromises]);

      alert(
        "SINC_EXITOSA: TODOS LOS PARÁMETROS HAN SIDO ACTUALIZADOS EN LA NUBE",
      );
    } catch (err) {
      console.error(err);
      setErrorLocal(
        "ERROR_CRÍTICO: FALLO AL GUARDAR CONFIGURACIÓN EN EL SERVIDOR",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="bg-black min-h-screen p-8 md:p-12 text-white font-['Inter'] selection:bg-[#ff8c00] selection:text-black">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8 border-b border-white/5 pb-10">
        <div>
          <h1 className={`${STYLES.title} text-5xl leading-none`}>
            CONFIGURACIÓN DE COSTOS <span className="text-[#ff8c00]">CORE</span>
          </h1>
          <p
            className={`${STYLES.tech} text-[10px] text-zinc-600 mt-6 tracking-[0.5em]`}
          >
            MOTOR DE LOGÍSTICA FINANCIERA // FEDECELL LABS
          </p>
        </div>
      ...
      <div className="space-y-12 pb-24">
        ...
        <div className={`${STYLES.glass} p-10`}>
          <h2
            className={`${STYLES.title} text-sm text-[#ff8c00] mb-10 flex items-center gap-4`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div>
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-6 top-1/2 -translate-y-1/2 text-zinc-700 font-black">
                  %
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    setConfigGlobal((prev) => ({
                      ...prev,
                      mpFee: parseFloat(e.target.value) || 0,
                    }))
                  }
              ...
              <p
                className={`${STYLES.tech} text-[9px] text-zinc-500 mt-4 uppercase tracking-widest`}
              >
                GESTIÓN PLATAFORMA DE PAGOS
              </p>
            </div>

            <div>
              <label className={STYLES.label}>
                Aumento de Precios Base Global (%)
              </label>
              <div className="relative">
                <input
                  type="number"
                  ...
                  onChange={(e) =>
                    setConfigGlobal((prev) => ({
                      ...prev,
                      aumentoGlobalProductos: parseFloat(e.target.value) || 0,
                    }))
                  }
              ...
              <p
                className={`${STYLES.tech} text-[9px] text-red-500 mt-4 flex items-center gap-2 font-bold`}
              >
                <FiAlertTriangle size={12} /> CRÍTICO: AFECTARÁ A TODOS LOS
        ...
        <div className={`${STYLES.glass} p-10`}>
          ...
          <div className="flex gap-4 mb-10">
            ...
            <button
              ...
              className={`${STYLES.buttonSecondary} py-2 !text-[11px]`}
            >
          ...
          <div className="space-y-6 max-h-[500px] overflow-y-auto custom-scrollbar pr-4">
            {searchResults.map((prod) => (
              <div
                key={prod.id}
                className="bg-white/5 border border-zinc-900 p-6"
              >
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="font-['Montserrat'] font-black text-white text-lg uppercase tracking-tighter">
                      {prod.nombre}
                    </h3>
                    <p className="text-[10px] text-zinc-500 font-mono uppercase mt-1">
                      {prod.marca} // SKU_{prod.id}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {prod.variantes?.map((variant, vIdx) => (
                    <div
                      key={vIdx}
                      className="bg-black/40 p-4 border border-zinc-800"
                    >
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-[10px] font-bold text-white uppercase">
                          {variant.color} - {variant.almacenamiento}
                        </span>
                      </div>

                      <div className="space-y-4">
                        <div>
                          <label className="text-[8px] text-zinc-600 block mb-1 uppercase">
                            PVP PÚBLICO
                          </label>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-700 text-xs">
                              $
                            </span>
                            <input
                              type="number"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {Object.entries(costosEnvio).map(([key, value]) => (
              <div key={key} className="bg-black border border-zinc-900 p-6">
                ...
                <div className="relative">
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 text-zinc-700 font-bold">
                    $
                  </span>
                  <input
                    type="number"
        ...
        <div className={`${STYLES.glass} p-10`}>
          ...
          <form
            onSubmit={handleAddAumento}
            className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-zinc-900/50 p-6 border border-zinc-800 mb-10"
          >
            <input
              type="text"
              name="banco"
            ...
            <div className="relative">
              <input
                ...
                className={STYLES.input}
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-700 font-bold">
                %
              </span>
            </div>
            <button
              ...
              className={`${STYLES.buttonPrimary} py-2 !text-[10px] flex items-center justify-center gap-2 disabled:opacity-20`}
            >
              <FiPlus /> AÑADIR REGLA
            </button>
          </form>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead
                className={`${STYLES.tech} text-[10px] text-zinc-600 border-b border-zinc-900`}
              >
                <tr>
              ...
              <tbody className="divide-y divide-zinc-900">
                {aumentosPorTarjeta.map((regla) => {
                  const isEditing =
                    reglaAEditar && reglaAEditar.id === regla.id;
                  return (
                    <tr
                      key={regla.id}
                      className={`transition-colors ${isEditing ? "bg-[#ff8c00]/5" : "hover:bg-white/[0.01]"}`}
                    >
                      {isEditing ? (
                        <>
                          <td className="p-4">
                            <input
                              type="text"
                              name="banco"
                              value={reglaAEditar.banco}
                              onChange={handleEditReglaChange}
                              className={`${STYLES.input} py-1 text-xs`}
                            />
                          </td>
                          <td className="p-4">
                            <input
                          ...
                          <td className="p-4">
                            <input
                              type="number"
                              name="porcentajeAumento"
                              value={reglaAEditar.porcentajeAumento}
                              onChange={handleEditReglaChange}
                              className={`${STYLES.input} py-1 text-xs`}
                            />
                          </td>
                        ...
                        <>
                          <td
                            className={`${STYLES.tech} p-4 text-white font-bold`}
                          >
                            {regla.banco}
                          </td>
                          <td className="p-4 text-zinc-400 font-['JetBrains_Mono']">
                            {regla.cuotas} CUOTAS
                          </td>
                          <td className="p-4 text-[#ff8c00] font-black font-['JetBrains_Mono']">
                            {regla.porcentajeAumento.toFixed(1)}%
                          </td>
                          <td className="p-4 text-right flex justify-end gap-4">
                            <button
                              onClick={() => handleEditStart(regla)}
                              disabled={!!reglaAEditar}
                            ...
                            <button
                              onClick={() => handleDeleteAumento(regla.id)}
                              disabled={!!reglaAEditar}
