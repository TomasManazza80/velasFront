...
const STYLES = {
  ...
  buttonPrimary:
    "bg-white text-black font-['Inter'] font-[900] uppercase tracking-widest py-4 px-8 rounded-none hover:bg-zinc-200 transition-all shadow-[0_10px_20px_rgba(255,255,255,0.05)]",
  buttonSecondary:
  ...
  tabInactive:
    "text-zinc-600 border-transparent hover:text-zinc-300 hover:bg-white/[0.01]",
};
...
const RegistroRevendedorContent = () => {
  ...
  const handleGuardarRevendedor = (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      alert(
        `SISTEMA: REVENDEDOR "${revendedor.nombre.toUpperCase()}" INDEXADO CORRECTAMENTE.`,
      );
    ...
    <div className={`${STYLES.glass} rounded-none overflow-hidden`}>
      ...
      <div className="px-8 py-6 border-b border-white/5 bg-white/[0.01]">
        <h2 className={`${STYLES.title} text-sm flex items-center gap-3`}>
          <FiUserCheck className="text-white" size={18} /> REGISTRO DE NUEVA
          ENTIDAD COMERCIAL
        </h2>
      </div>
      ...
      <div className="p-4 md:p-10">
        <form onSubmit={handleGuardarRevendedor} className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="dniCuit"
                  value={revendedor.dniCuit}
                  onChange={handleInputChange}
                  className={`${STYLES.input} ${STYLES.tech}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    name="nivel"
                    value={revendedor.nivel}
                    onChange={handleInputChange}
                    className={`${STYLES.input} pl-12 appearance-none cursor-pointer`}
                  >
                    {nivelesRevendedor.map((n) => (
              ...
              <div>
                ...
                <input
                  type="number"
                  name="margenGanancia"
                  value={revendedor.margenGanancia}
                  onChange={handleInputChange}
                  className={`${STYLES.input} ${STYLES.tech} text-white`}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="number"
                    name="limiteCredito"
                    value={revendedor.limiteCredito}
                    onChange={handleInputChange}
                    className={`${STYLES.input} pl-12 ${STYLES.tech}`}
                    placeholder="0.00"
                  />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="email"
                    name="email"
                    value={revendedor.email}
                    onChange={handleInputChange}
                    className={`${STYLES.input} pl-12 lowercase`}
              ...
              <div>
                ...
                <input
                  type="text"
                  name="zonaVenta"
                  value={revendedor.zonaVenta}
                  onChange={handleInputChange}
                  className={STYLES.input}
                  placeholder="EJ: SANTA FE CENTRO"
                />
              </div>
              <div>
                <label className={STYLES.label}>
                  Dirección Física de Depósito
                </label>
                <input
                  type="text"
                  name="direccion"
                  value={revendedor.direccion}
                  onChange={handleInputChange}
                  className={STYLES.input}
          ...
          <div className="pt-4">
            <label className={STYLES.label}>
              Notas Técnicas Adicionales del Operador
            </label>
            <textarea
              name="informacion"
          ...
          <div className="flex flex-col md:flex-row justify-end gap-4 pt-10 border-t border-white/5">
            ...
            >
              ...
              {loading
                ? "PROCESANDO DATOS..."
                : "CONFIRMAR REGISTRO DE REVENDEDOR"}
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12 gap-6">
        <div>
          <h1 className={`${STYLES.title} text-3xl md:text-5xl leading-none`}>
            GESTIÓN DE <span className="text-white">REVENDEDORES</span>
          </h1>
          <p
            className={`${STYLES.tech} text-[10px] text-zinc-600 mt-6 tracking-[0.5em]`}
          >
            OUTSIDE_SALES_NETWORK // CORE_V1.0
          </p>
        </div>
        ...
        {activeTab !== "registro" && (
          <div className="flex flex-col items-center justify-center py-32 text-zinc-800 border border-dashed border-zinc-900">
            ...
            <p className={`${STYLES.tech} text-[10px] tracking-[0.6em]`}>
              MÓDULO EN DESARROLLO - SISTEMA PENDIENTE
            </p>
          </div>
        )}
      </div>
