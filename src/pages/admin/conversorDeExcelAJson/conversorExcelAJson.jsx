...
const conversorExcelAJson = () => {
  ...
  const [remitoId, setRemitoId] = useState(
    `REM-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
  );
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      rawData.forEach((row) => {
        ...
        const costo = cleanNum(
          row["variantes.costoDeCompra"] || row.precioCompra,
        );
        const stock = cleanNum(row["variantes.stock"]);
        const pVenta = cleanNum(
          row["variantes.precioAlPublico"] || row.precioVenta,
        );
        ...
        if (!productsMap[nombre]) {
          productsMap[nombre] = {
            ...
            imagenes: row.imagenes
              ? row.imagenes.split(",").map((img) => img.trim())
              : [],
            variantes: [],
            cantidad: 0,
            costoCompra: costo,
        ...
        productsMap[nombre].variantes.push({
          ...
          precioMayorista: cleanNum(
            row["variantes.precioMayorista"] || row.precioMayorista,
          ),
          precioRevendedor: cleanNum(
            row["variantes.precioRevendedor"] || row.precioRevendedor,
          ),
        });
          ...
          <div className="animate-in fade-in slide-in-from-bottom-5 duration-700">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              <button
                ...
                className="bg-white text-black font-black py-5 rounded-2xl uppercase text-[11px] tracking-widest hover:bg-orange-500 hover:text-white transition-all"
              >
                {copied ? (
              ...
              <button
                onClick={() => {
                  const blob = new Blob([JSON.stringify(remitoData, null, 2)], {
