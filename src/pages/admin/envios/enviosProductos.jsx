...
const STYLES = {
  ...
  label:
    "font-['Inter'] text-[11px] font-bold text-white uppercase tracking-[0.2em] mb-2 block",
  tech: "font-['Inter'] tracking-widest uppercase",
  glass: "bg-zinc-900/40 backdrop-blur-md border border-zinc-800",
  buttonAction:
    "bg-white text-gray-900 font-['Inter'] font-black text-[11px] uppercase tracking-widest transition-all hover:bg-zinc-200",
  input:
    "w-full bg-black border border-zinc-800 py-4 px-6 text-sm text-white font-['Inter'] focus:border-white outline-none transition-all",
};
const ESTADOS = {
  PENDIENTE: {
    label: "PENDIENTE",
  ...
  const handleEstado = async (e, nuevoEstado) => {
    ...
    try {
      if (orden.originalIds && orden.originalIds.length > 0) {
        await Promise.all(
          orden.originalIds.map((id) =>
            axios.patch(`${API_URL}/ecommerce/pedidos/${id}/estado`, {
              dispatchStatus: nuevoEstado,
            }),
          ),
        );
      } else {
        await axios.patch(`${API_URL}/ecommerce/pedidos/${orden.id}/estado`, {
          dispatchStatus: nuevoEstado,
        });
      }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between p-4 border-b border-zinc-900">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {orden.isGrouped && (
              <span
                className={`text-[7px] font-bold px-1.5 py-0.5 rounded-none border text-white bg-white/10 border-white/30`}
              >
                {" "}
                ENCOMIENDA{" "}
              </span>
            ...
            <span
              className={`text-[7px] font-bold px-1.5 py-0.5 rounded-none border ${isMayorista ? "text-white bg-white/10 border-white/30" : "text-gray-400 bg-zinc-900 border-zinc-700"}`}
            >
              {" "}
              {isMayorista ? "MAYOR" : "ECOM"}{" "}
            </span>{" "}
            <span
              className={`text-[7px] font-bold px-1.5 py-0.5 rounded-none border text-zinc-300 bg-white/5 border-white/10`}
            >
              {" "}
          ...
          <p
            className={`${STYLES.tech} text-[11px] font-black text-white group-hover:text-zinc-300 transition-colors uppercase`}
          >
            {" "}
            {orden.name || "CLIENTE S/N"}{" "}
          </p>{" "}
          {orden.cellphone && (
            <p className="text-[9px] text-gray-600 flex items-center gap-1 mt-0.5 font-bold">
              {" "}
        ...
        <div className="text-right flex-shrink-0 ml-2">
          {" "}
          <p className={`${STYLES.tech} text-sm font-black text-white`}>
      ...
      {orden.address && (
        <div className="px-4 py-2 flex items-center gap-1.5">
          ...
          <p className="text-[9px] text-gray-500 font-bold uppercase truncate">
            {" "}
            {[orden.address, orden.city, orden.province]
              .filter(Boolean)
              .join(", ")}{" "}
          </p>{" "}
        </div>
      ...
      <div className="px-4 pb-3">
        {" "}
        <p className="text-[9px] text-gray-600 font-bold mb-1 leading-relaxed">
          {" "}
          {items.length > 0
            ? items
                .map((i) => {
                  ...
                  if (i.storage || i.almacenamiento)
                    dt.push(i.storage || i.almacenamiento);
                  return `${i.quantity}x ${i.title} ${dt.length > 0 ? `[${dt.join(", ")}]` : ""}`;
                })
                .join(", ")
                .substring(0, 80) + (items.length > 2 ? "..." : "")
            : "Sin productos"}{" "}
        </p>{" "}
      </div>{" "}
        ...
        {Object.entries(ESTADOS).map(([key, cfg]) => {
          ...
          return (
            <button
              ...
              className={`flex-1 py-2 text-[7px] font-black tracking-widest ${STYLES.tech} border-r border-zinc-900 last:border-r-0 transition-all ${isActive ? `${cfg.bg} ${cfg.color}` : "text-gray-600 hover:text-white"}`}
            >
              {" "}
              {cfg.label}{" "}
            </button>
...
const ShipmentLogRow = ({ env, cambiarEstado }) => {
  ...
  return (
    <div
      className={`${STYLES.glass} relative flex flex-col transition-all border-l-4 ${Config.border} font-['Inter']`}
    >
      {" "}
      <div className="flex flex-col md:flex-row items-stretch">
          ...
          <div className="w-full md:w-64 bg-zinc-900/50 p-4 flex flex-col justify-center items-center border-b md:border-b-0 md:border-r border-zinc-800 animate-in slide-in-from-left duration-300">
            ...
            {Array.isArray(env.items) && env.items.length > 0 ? (
              <div className="relative w-32 h-32">
                ...
                {env.items[0].image ? (
                  <img
                    src={env.items[0].image}
                    alt="product"
                    className="w-full h-full object-cover filter contrast-125 grayscale hover:grayscale-0 transition-all duration-500 shadow-2xl"
                  />
                ) : (
                ...
                {env.items.length > 1 && (
                  <div className="absolute -bottom-2 -right-2 bg-white text-gray-900 text-[10px] font-black px-2 py-1 shadow-lg">
                    {" "}
                    +{env.items.length - 1}{" "}
                  </div>
                )}{" "}
        ...
        <div className="flex-1 p-6 space-y-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <div>
              {" "}
              <span
                className={`${STYLES.tech} text-[10px] text-white font-black mb-1 block`}
              >
                REF: #ORD_{env.id}
              </span>{" "}
              <h3
                className={`${STYLES.tech} text-lg font-black text-white uppercase`}
              >
                {env.name || "CLIENTE_S_N"}
              </h3>{" "}
            </div>{" "}
            <div className="text-right">
              {" "}
              <p className={`${STYLES.tech} text-xl font-black text-white`}>
                {formatMoneda(env.total)}
              </p>{" "}
              <span
                className={`${STYLES.tech} text-[9px] text-gray-500 font-bold`}
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {expandido ? (
                ...
                <p className="text-[10px] text-zinc-300 font-bold leading-relaxed uppercase">
                  ...
                  {Array.isArray(env.items) && env.items.length > 0
                    ? env.items
                        .map(
                          (i) =>
                            `${i.quantity}x ${i.title} [${i.color || i.almacenamiento || "—"}]`,
                        )
                        .join(" | ")
              ...
              <div className="flex items-center">
                {" "}
                <span
                  className={`${STYLES.tech} text-[8px] text-gray-600 italic font-bold`}
                >
                  Click en VER_DETALLE para ver productos
                </span>{" "}
              </div>
            )}{" "}
          </div>{" "}
          {env.tracking && (
            <div className="bg-white/5 border border-white/10 p-2 flex items-center justify-between">
              {" "}
              <span
                className={`${STYLES.tech} text-[9px] text-gray-400 font-bold`}
              >
                TRACKING_ID:
              </span>{" "}
              <span
                className={`${STYLES.tech} text-[10px] text-white font-black`}
              >
                {env.tracking}
              </span>{" "}
            </div>
        ...
        <div className="w-full md:w-56 p-6 flex flex-col justify-between gap-4 border-t md:border-t-0 md:border-l border-zinc-800 bg-white/[0.01]">
          ...
          <div className="space-y-3">
            {" "}
            <p className="text-[9px] text-gray-600 uppercase font-black tracking-widest mb-2">
              Power_Management
            </p>{" "}
            <div className="relative">
              ...
              >
                ...
                <option
                  value="PENDIENTE"
                  className="bg-[#0A58CA] text-white font-medium text-sm rounded-xl shadow-sm hover:bg-[#084298] transition-all"
                >
                  PENDIENTE
                </option>{" "}
                <option
              ...
              <div
                className={`absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none ${Config.color}`}
              >
                {" "}
                <Config.icon size={16} />{" "}
              </div>{" "}
          ...
          <button
            ...
            className={`w-full py-4 transition-all flex items-center justify-center gap-3 border ${expandido ? "bg-white text-gray-900 border-white" : "bg-zinc-900 border-zinc-800 text-gray-400 hover:text-white hover:border-white"}`}
          >
            {" "}
...
const EnviosProductos = () => {
  ...
  const handleEcommerceEstadoCambio = (id, nuevoEstado) => {
    setPedidosEcommerce((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, dispatchStatus: nuevoEstado } : p,
      ),
    );
    fetchEnvios();
    /* Refrescar lista de gestión */
  };
  const pedidosSyncFiltrados = useMemo(() => {
    const busq = busquedaEcommerce.toLowerCase();
    return pedidosEcommerce.filter((p) => {
      if (p.shippingOption && p.shippingOption.toLowerCase().includes("retiro"))
        return false;
      const matchBusq =
        !busq ||
      ...
      const matchEstado =
        filtroEcommerce === "TODOS" || p.dispatchStatus === filtroEcommerce;
    ...
    const descDetallada =
      items
        .map((p) => {
          ...
          if (p.storage || p.almacenamiento)
            details.push(`Cap: ${p.storage || p.almacenamiento}`);
          return `${p.quantity}x ${p.title} ${details.length > 0 ? `(${details.join(", ")})` : ""}`;
        })
        .join(" | ") || "Sin descripción";
    setNuevoEnvio({
      cliente: pedido.name || "",
      destino: [pedido.address, pedido.city, pedido.province, pedido.postalCode]
        .filter(Boolean)
        .join(", "),
      metodo: pedido.shippingOption || "ECOMMERCE_ONLINE",
      productosCount: items.reduce(
        (acc, p) => acc + (Number(p.quantity) || 1),
        0,
      ),
      monto: pedido.total || "",
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    try {
      await axios.patch(`${API_URL}/ecommerce/pedidos/${id}/estado`, {
        dispatchStatus: nuevoEstado,
      });
      setEnvios((prev) =>
        prev.map((envio) =>
          envio.id === id ? { ...envio, dispatchStatus: nuevoEstado } : envio,
        ),
      );
    } catch (err) {
      console.error("Error actualizando estado:", err);
  ...
  const autorizarDespacho = async (e) => {
    ...
    try {
      if (pedidoEnEdicion) {
        /* Si viene de un pedido existente, actualizamos sus datos await */
        axios.patch(
          `${API_URL}/ecommerce/pedidos/${pedidoEnEdicion.id}/estado`,
          {
            ...
            dispatchStatus: "EN_CAMINO",
            /* Al autorizar pasa a EN CAMINO */
          },
        );
      } else {
        /* Si es un envío manual nuevo await */
        axios.post(`${API_URL}/ecommerce/pedidos`, {
          name: nuevoEnvio.cliente,
          address: nuevoEnvio.destino,
          cellphone: nuevoEnvio.telefono,
  ...
  const filtrados = useMemo(() => {
    return envios.filter((e) => {
      ...
      const coincideEstado =
        filtroEstado === "TODOS" || e.dispatchStatus === filtroEstado;
      return coincideBusqueda && coincideEstado;
    });
  }, [envios, busqueda, filtroEstado]);
  return (
    <div className="p-4 md:p-10 bg-black min-h-screen text-gray-900 rounded-none space-y-12 font-['Inter'] selection:bg-[#F4F7FE] selection:text-gray-900">
      {" "}
      {/* HEADER MASTER */}{" "}
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        ...
        <div>
          ...
          <h1 className={`${STYLES.title} text-3xl md:text-5xl leading-none`}>
            LOGÍSTICA_<span className="text-white">CENTRAL</span>
          </h1>{" "}
          <p
            className={`${STYLES.tech} text-[10px] text-gray-600 mt-4 tracking-[0.5em] font-bold`}
          >
            CORE_DISPATCH_SYSTEM /* FEDECELL_LABS
          </p>{" "}
        </div>{" "}
        */
        <div className={`${STYLES.glass} p-1 flex gap-1`}>
          ...
          <button
            ...
            className={`px-8 py-3 text-[10px] ${STYLES.tech} font-black transition-all ${tabActiva === "listado" ? "bg-white text-gray-900 shadow-lg" : "text-gray-500 hover:text-white"}`}
          >
            {" "}
          ...
          <button
            ...
            className={`px-8 py-3 text-[10px] ${STYLES.tech} font-black transition-all ${tabActiva === "nuevo" ? "bg-white text-gray-900 shadow-lg" : "text-gray-500 hover:text-white"}`}
          >
            {" "}
      ...
      {tabActiva === "listado" ? (
        /* --- VISTA: LISTADO DE ENVÍOS MANUALES --- */ <div className="animate-in fade-in duration-500 space-y-10">
          {" "}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
            ...
            <div className="lg:col-span-8 relative">
              ...
              <input
                type="text"
                placeholder="BUSCAR_CLIENTE_O_ORDEN_ID..."
                className={`${STYLES.input} pl-16 py-5 ${STYLES.tech} text-xs`}
                value={busqueda}
            ...
            <div className="lg:col-span-4 flex gap-2">
              ...
              {["TODOS", "PENDIENTE", "EN_CAMINO", "RECIBIDO"].map((est) => (
                <button
                  ...
                  className={`flex-1 py-3 text-[9px] ${STYLES.tech} font-black border transition-all ${filtroEstado === est ? "bg-white text-gray-900 border-white" : "bg-transparent text-gray-600 border-zinc-800 hover:border-white"}`}
                >
                  {" "}
                  {est}{" "}
                </button>
            ...
            {cargandoEnvios ? (
              <div className="py-20 flex flex-col items-center justify-center text-zinc-700">
                ...
                <FiLoader
                  ...
                  size={48}
                />{" "}
                <span className={`${STYLES.tech} text-xs font-bold uppercase`}>
                  Sincronizando_Registros...
                </span>{" "}
              </div>
            ) : filtrados.length > 0 ? (
              filtrados.map((env) => (
              ...
              <div className="py-20 text-center opacity-20">
                ...
                <p className={`${STYLES.tech} text-xs font-bold`}>
                  SISTEMA_SIN_REGISTROS_ACTIVOS
                </p>{" "}
              </div>
            )}{" "}
          </div>{" "}
        ...
        /* --- VISTA: PEDIDOS ONLINE (SYNC + FORMULARIO) --- */ <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 animate-in slide-in-from-bottom-8 duration-700">
          {" "}
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
              {" "}
              <h3
                className={`text-[11px] ${STYLES.tech} font-black text-white flex items-center gap-3`}
              >
                {" "}
                <FiDownloadCloud size={18} /> SYNC_ECOMMERCE_PEDIDOS{" "}
              </h3>{" "}
              <button
                onClick={fetchEcommerce}
                className="bg-zinc-900 border border-zinc-800 text-[9px] p-2 hover:border-white hover:text-white transition-all"
              >
                {" "}
                <FiRefreshCw
                  size={13}
            ...
            <div className="flex gap-1">
              ...
              {["TODOS", "PENDIENTE", "EN_CAMINO", "RECIBIDO"].map((est) => (
                <button
                  ...
                  className={`flex-1 py-1.5 text-[7px] ${STYLES.tech} font-black border transition-all ${filtroEcommerce === est ? "bg-white text-gray-900 border-white" : "text-gray-500 border-zinc-800 hover:border-zinc-500"}`}
                >
                  {" "}
                  {est.replace("_", " ")}{" "}
                </button>
              ...
              {cargandoEcommerce ? (
                <div className="h-full flex flex-col items-center justify-center text-zinc-700">
                  ...
                  <FiLoader
                    ...
                    size={32}
                  />{" "}
                  <span className={`${STYLES.tech} text-[10px] font-bold`}>
                    DESCARGANDO_DATOS...
                  </span>{" "}
                </div>
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-20 text-center">
                  ...
                  <p className={`${STYLES.tech} text-[10px] font-bold`}>
                    SIN_PEDIDOS_DISPONIBLES
                  </p>{" "}
                </div>
              )}{" "}
            </div>{" "}
            ...
            {pedidosSyncFiltrados.length > 0 && (
              <p
                className={`${STYLES.tech} text-[9px] text-gray-600 font-bold text-center uppercase`}
              >
                {" "}
                {pedidosSyncFiltrados.length} Paquetes /*
                CLICK_PARA_IMPORTAR{" "}
              </p>
            )}{" "}
          </div>{" "}
          ...
          <div className="lg:col-span-8">
            {" "}
            <div
              className={`${STYLES.glass} p-6 md:p-12 bg-zinc-900/20 shadow-2xl relative overflow-hidden border-zinc-800`}
            >
              {" "}
              ...
              <div className="flex justify-between items-start mb-12">
                {" "}
                <h2
                  className={`${STYLES.title} text-3xl flex items-center gap-4`}
                >
                  {" "}
                ...
                {pedidoEnEdicion && (
                  <span
                    className={`${STYLES.tech} text-[10px] bg-white/10 text-white border border-white/20 px-3 py-1 font-black`}
                  >
                    {" "}
              ...
              >
                ...
                <div className="col-span-2 md:col-span-1 space-y-3">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNuevoEnvio({ ...nuevoEnvio, cliente: e.target.value })
                    }
                  />{" "}
                </div>{" "}
                <div className="col-span-2 md:col-span-1 space-y-3">
                  ...
                  <input
                    className={`${STYLES.input} ${STYLES.tech} font-bold`}
                    value={nuevoEnvio.telefono}
                    onChange={(e) =>
                      setNuevoEnvio({ ...nuevoEnvio, telefono: e.target.value })
                    }
                  />{" "}
                </div>{" "}
                <div className="col-span-1 space-y-3">
                  ...
                  <input
                    required
                    type="number"
                    className={`${STYLES.input} ${STYLES.tech} !text-white font-black text-xl`}
                    value={nuevoEnvio.monto}
                    onChange={(e) =>
                      setNuevoEnvio({ ...nuevoEnvio, monto: e.target.value })
                    }
                  />{" "}
                </div>{" "}
                <div className="col-span-1 space-y-3">
                  {" "}
                  <label className={STYLES.label}>
                    ID_DE_TRACKING (OPCIONAL)
                  </label>{" "}
                  <input
                    className={`${STYLES.input} ${STYLES.tech} font-bold border-zinc-800 text-white focus:border-white`}
                    placeholder="EJ: AR-99281-X"
                    value={nuevoEnvio.tracking}
                    onChange={(e) =>
                      setNuevoEnvio({ ...nuevoEnvio, tracking: e.target.value })
                    }
                  />{" "}
                </div>{" "}
                <div className="col-span-2 space-y-3">
                  {" "}
                  <label className={STYLES.label}>
                    Dirección_De_Destino_Geolocalizada
                  </label>{" "}
                  <input
                    required
                    className={`${STYLES.input} uppercase font-bold`}
                    value={nuevoEnvio.destino}
                    onChange={(e) =>
                      setNuevoEnvio({ ...nuevoEnvio, destino: e.target.value })
                    }
                  />{" "}
                </div>{" "}
                <div className="col-span-2 space-y-3">
                  {" "}
                  <label className={STYLES.label}>
                    Especificación_Hardware_Y_Contenido (DETALLES_DEL_PAQUETE)
                  </label>{" "}
                  <textarea
                    required
                    className={`${STYLES.input} min-h-[120px] resize-none uppercase text-[10px] leading-relaxed font-bold border-zinc-800`}
                    value={nuevoEnvio.descripcion}
                    onChange={(e) =>
                      setNuevoEnvio({
                        ...nuevoEnvio,
                ...
                >
                  ...
                  {pedidoEnEdicion
                    ? "ACTUALIZAR_Y_AUTORIZAR_DESPACHO"
                    : "GENERAR_NUEVO_DESPACHO_MANUAL"}{" "}
                </button>{" "}
                {pedidoEnEdicion && (
                  <button
                    type="button"
                    ...
                    className={`col-span-2 py-3 text-[10px] ${STYLES.tech} text-gray-500 hover:text-white transition-all uppercase font-bold tracking-widest`}
                  >
                    {" "}
