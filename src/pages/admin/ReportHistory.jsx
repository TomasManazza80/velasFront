...
const ReportHistory = ({
  recaudaciones,
  recaudacionesExpandidas,
  toggleExpandirRecaudacion,
  eliminarRecaudacion,
  loading,
}) => {
  return (
      ...
      <div className="px-6 py-4 border-b border-gray-700 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-medium text-white">
            Reporte de Ganancias (Historial)
          </h2>
          <p className="text-sm text-gray-400">
            Historial de cierres de caja y recaudaciones registradas. Total de
            registros: {recaudaciones.length}
          </p>
        </div>
      </div>
      <div className="divide-y divide-gray-700">
        {recaudaciones.length === 0 ? (
          ...
          recaudaciones.map((recaudacion) => (
            <div
              key={recaudacion.id}
              className="p-6 hover:bg-gray-700 transition duration-150"
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="mt-2 text-sm text-gray-400">
                    <p>
                      ...
                      ${recaudacion.montoRecaudado.toLocaleString()}
                    </p>
                    <p>
                      <span className="font-medium text-gray-300">
                        Productos Vendidos:
                      </span>{" "}
                      {recaudacion.productosVendidos.length}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col items-end space-y-2">
                  <div className="text-sm text-gray-500">
                    <p>
                      Fecha:{" "}
                      {new Date(recaudacion.createdAt).toLocaleDateString(
                        "es-AR",
                      )}
                    </p>
                  </div>
                  <div className="flex space-x-2">
                    ...
                    >
                      {recaudacionesExpandidas[recaudacion.id]
                        ? "Ocultar productos"
                        : "Ver productos"}
                    </button>
                    <button
              ...
              {recaudacionesExpandidas[recaudacion.id] &&
                recaudacion.productosVendidos.length > 0 && (
